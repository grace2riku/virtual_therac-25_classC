...
| Step | 内容 | 予定 |
|------|------|------|
| Step 19 | **Inc.1 中核ユニット実装(UNIT-201 SafetyCoreOrchestrator)**。`LifecycleState` 8 状態機械(Init / SelfCheck / Idle / PrescriptionSet / Ready / BeamOn / Halted / Error)+ 遷移許可表 + `MessageBus`(UNIT-401)経由のイベントループ + メッセージディスパッチ。UTPR v0.3 へ昇格 + 該当 UT 追加(状態遷移網羅 + 不正遷移拒否 + イベントループ動作)| 次回作業時 |
| Step 20+ | **残ユニット実装**(UNIT-202〜206 / 208 各 Manager 群、UNIT-301〜304 Hardware Simulator、UNIT-402 InterProcessChannel)+ UT 追加。Inc.2/4 該当の UNIT-207/209/210 / 101〜104 は空殻実装(SDD §4.8/4.10/4.11 と整合)| Step 19 完了後 |
