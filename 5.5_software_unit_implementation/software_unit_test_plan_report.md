...
| UT-204-27 | `DoseManager_HZ007.AtomicsAreLockFree` | (runtime + static_assert) | `std::atomic<uint64_t>` + `std::atomic<bool>` 両方 lock-free | 構造的(HZ-007)| SDD §7 SOUP-003/004 | **RCM-002 / HZ-002 / HZ-007** |
...
| **UT-204-30** | **`DoseManager_Concurrency.ProducerProducerReader`** | **4 producer × 10000 パルス + 1 reader 並行** | **累積=40000 cGy、reached=false(40000 < 100000)、reader race-free に load**(`tsan` プリセットで race detection 0)| **並行処理(TSan 必須)** | SRS-D-009 / SDD §4.5 / §9 SEP-003 | **RCM-002 / RCM-004 / HZ-002** |
...
| 実施期間 | 2026-05-01 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: 本 Step 22 本体コミット(SHA は push 後事後追記) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_dose_manager_tests` 32 件追加。GitHub Actions cpp-build ワークフロー(run id は push 後追記)|
| **CI 結果** | **全 9 ジョブ success ✅ 期待**(push 後 `gh run view` で確定後、SHA 追記事後処理で記録)。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-204-30(4 producer × 10000 パルス + 1 reader 並行)race detection 0 で Pass を期待**(`std::atomic<uint64_t>::fetch_add(acq_rel)` + `std::atomic<bool>` の release-acquire ordering の正しさが両コンパイラで機械検証成立)。**UT-204-21 で 10^9 パルス積算でも uint64_t::max() 余裕(5,800 万年連続照射相当)を機械検証成立**(Yakima Class3 オーバフロー類型の構造的不可能化を実装層で完成)。**UT-204-25 で SDD §4.5 表との完全一致を 8 lifecycle × 3 ドーズ値 = 24 組合せ網羅機械検証成立**(SDD ↔ 実装の構造的乖離防止メカニズムを 5 ユニット目に拡大)|
...
| (Step 22 / CR-0010 範囲)| (ローカル骨格ビルド成功、CI 全 9 ジョブ Pass は push 後事後確認)| Minor 以下 | — | 進行中(2026-05-01、SHA 追記事後処理コミットで確定予定)|
...
### UNIT-204(本 Step 22 / CR-0010、HZ-005 直接 + HZ-003 構造的排除の中核、CI 実績は push 後 SHA 追記事後処理で確定)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-204 | UT-204-01〜02 | SDD §4.5 / §6.5(初期状態 / rate 注入)| RCM-003 / RCM-008 | 全 9 ジョブ Pass 期待(2026-05-01、push 後 run id 確定)|
...
| UNIT-204 | **UT-204-30** | SRS-D-009 / SDD §4.5 / §9 SEP-003(4 producer + 1 reader) | **RCM-002 / RCM-004 / HZ-002** | **`tsan` プリセットで 4 producer × 10000 + 1 reader race 0 確認(push 後)** |
