...
**バージョン:** 0.4
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2 昇格)/ 2026-04-30(v0.3 昇格)/ 2026-04-30(v0.4 昇格)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005(GitHub Issue #5、初版 UNIT-200)/ CR-0006(GitHub Issue #6、v0.2 UNIT-401 追加)/ CR-0007(GitHub Issue #7、v0.3 UNIT-201 追加)/ CR-0008(GitHub Issue #8、v0.4 UNIT-202 追加)
...
>
> **本 v0.4(Step 20 / CR-0008)では UNIT-202 TreatmentModeManager(HZ-001 直接対応の中核ユニット)を追加**。UNIT-203〜210 / 301〜304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| [11] | 構成アイテム一覧(CIL-TH25-001) | 0.12 |
| [12] | CCB 運用規程(CCB-TH25-001) | 0.1 |
| [13] | 変更要求台帳(CRR-TH25-001) | 0.10 |
...
|------------|------|------|------------------|----------------|
| **UNIT-200** | **CommonTypes** | k-abe | `src/th25_ctrl/include/th25_ctrl/common_types.hpp` | **v0.1 で計画 + 実施記録(Step 17 / CR-0005)、v0.2/v0.3/v0.4 で維持** |
| **UNIT-401** | **InProcessQueue** | k-abe | `src/th25_ctrl/include/th25_ctrl/in_process_queue.hpp` | **v0.2 で計画 + 実施記録(Step 18 / CR-0006)、v0.3/v0.4 で維持** |
| **UNIT-201** | **SafetyCoreOrchestrator** | k-abe | `src/th25_ctrl/include/th25_ctrl/safety_core_orchestrator.hpp` + `src/safety_core_orchestrator.cpp` | **v0.3 で計画 + 実施記録(Step 19 / CR-0007)、v0.4 で維持** |
| **UNIT-202** | **TreatmentModeManager** | k-abe | `src/th25_ctrl/include/th25_ctrl/treatment_mode_manager.hpp` + `src/treatment_mode_manager.cpp` | **本 v0.4 で計画 + 実施記録(Step 20 / CR-0008、HZ-001 直接対応の中核)** |
| UNIT-101〜104 | Operator UI 群 | k-abe | `src/th25_ui/...`(Inc.4 で追加予定) | (Inc.4 SDD 改訂時に追加) |
| UNIT-203 | BeamController | k-abe | `src/th25_ctrl/...` | (Step 21+ 後続で追加予定) |
| UNIT-204 | DoseManager | k-abe | `src/th25_ctrl/...` | (Step 21+ 後続で追加予定) |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202)= 87 件**
...
| 実施期間 | 2026-04-30 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 20 push 後に SHA 追記事後処理で記録) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_treatment_mode_manager_tests` 26 件追加。GitHub Actions cpp-build ワークフロー(本 Step push 後に run id 確定)|
| **CI 結果** | (本 Step push 後に SHA 追記事後処理で確定。期待値: 全 9 ジョブ success ✅、`clang-17/tsan` + `gcc-13/tsan` で UT-202-21(2 writer + 1 reader 並行)race detection 0、UT-202-25 で SDD §4.3 表との完全一致を 9 組合せ網羅試験で機械的検証成立、UT-202-18 で Therac-25 Tyler 事故型「Electron API に XRay モード」の構造的拒否を機械検証成立)|
...
| 試験 ID | ローカル骨格ビルド | CI cpp-build マトリクス(2026-04-30、run id 後追記)| 備考 |
|--------|------------------|----------------------------|------|
| UT-202-01〜02(初期状態 / 明示初期化)| 構文 OK | (push 後追記)| 既定 Light、明示 Electron/XRay |
| UT-202-03〜08(モード遷移正常系 6 件)| 構文 OK | (push 後追記)| Light↔Electron↔XRay の任意遷移 + Light 戻り + 同モード no-op |
| UT-202-09〜11(BeamState != Off で要求拒否)| 構文 OK | (push 後追記)| **HZ-001 直接対応**(Therac-25 Tyler 事故型「BeamState 確認漏れ」の構造的拒否)|
| UT-202-12〜15(エネルギー範囲外拒否 4 件)| 構文 OK | (push 後追記)| Electron(0.99/25.01 MeV)、XRay(4.99/25.01 MV)|
| UT-202-16〜17(エネルギー境界値受入)| 構文 OK | (push 後追記)| 境界値 ±0.01 まで網羅 |
| **UT-202-18〜19**(API ガード)| 構文 OK | (push 後追記)| **Electron API に XRay モード / XRay API に Electron モードを構造的拒否**(Therac-25 Tyler 事故型「Electron モードで X 線エネルギー」発現経路の阻止) |
| UT-202-20(強い型 compile-time)| 構文 OK | (push 後追記)| Energy_MeV ↔ Energy_MV 暗黙変換不可 |
| **UT-202-21** | 構文 OK | (push 後追記。期待値: `clang-17/tsan` + `gcc-13/tsan` で 2 writer + 1 reader × 1000 イテレーション race detection 0)| **HZ-002 機械検証**(`std::atomic<TreatmentMode>` の release-acquire ordering)|
| UT-202-22(コピー/ムーブ禁止)| 構文 OK | (push 後追記)| 4 trait compile-time |
| UT-202-23(lock-free 表明)| **`static_assert(std::atomic<TreatmentMode>::is_always_lock_free)` 通過** | (push 後追記)| HZ-007 構造的予防の build 時 fail-stop |
| UT-202-24(verify_mode_consistency)| 構文 OK | (push 後追記)| Step 21+ の TurntableManager 結線で 3 系統センサ依存判定に拡張予定 |
| **UT-202-25**(網羅試験 9 組合せ)| 構文 OK | (push 後追記)| **SDD §4.3 表との完全一致を機械検証**(全 9 通り許可)|
| UT-202-26(防御的 enum 値拒否)| 構文 OK | (push 後追記)| キャスト経由の不正値拒否 |
| (clang-tidy 全ファイル)| — | (push 後追記)| MISRA C++ 2023 関連チェック完全 Pass を期待 |

> **本表の中核成果物達成(計画段階):** UT-202-09〜11 / UT-202-18〜19 の構造的拒否は、Therac-25 Tyler 事故の三重エラー「Electron モードで 25 MV X 線エネルギー」「ターンテーブル X 線位置未到達」「BeamState 確認漏れ」の発現経路を、本 UNIT-202 の API ガード + BeamState 事前条件 + エネルギー範囲検証で機械的に阻止する設計を確立(RCM-001 / HZ-001 直接対応の中核)。UT-202-25 の 9 組合せ網羅試験で SDD §4.3 表との構造的乖離を防ぐ仕組みを 4 ユニット目に拡大。UT-202-21 で `std::atomic<TreatmentMode>` の race-free 性を Inc.1 オーケストレータ層 + Manager 層で検証可能にした(HZ-002 機械的予防の継続)。

### 8.3 カバレッジ実績

| ユニット ID | ステートメント | 分岐 | MC/DC | 備考 |
|------------|--------------|------|-------|------|
| UNIT-200 | (llvm-cov 未計測、Step 20+ 整備予定) | 同上 | — | UT 設計上は全機能を網羅(本書 §7.2 / §7.3) |
| UNIT-401 | (llvm-cov 未計測、Step 20+ 整備予定) | 同上 | — | UT 設計上は全公開 API + 境界値 + wrap-around + concurrent を網羅(本書 §7.2 / §7.3)|
| UNIT-201 | (llvm-cov 未計測、Step 21+ 整備予定) | 同上 | — | UT 設計上は公開 API 4 種 + 状態遷移 88 組合せ網羅 + イベントループ統合 + 並行処理を網羅(本書 §7.2 / §7.3) |
| UNIT-202 | (llvm-cov 未計測、Step 21+ 整備予定) | 同上 | — | UT 設計上は公開 API 5 種 + モード遷移 9 組合せ網羅 + BeamState/エネルギー範囲ガード + 強い型 compile-time + 並行処理を網羅(本書 §7.2 / §7.3) |
...
| (Step 20 / CR-0008 範囲)| (本 Step push 後に CI 結果に応じて追記)| | | |
...
- **UNIT-202 と TurntableManager / BendingMagnetManager の結線**: 本 v0.4 では UNIT-202 単体のモード状態 + 遷移許可 + エネルギー範囲検証を実装。SDD §4.3 事後条件「TurntableManager::move_to + BendingMagnetManager::set_current のスケジュール」と `verify_mode_consistency` の「3 系統センサ依存判定」は Step 21+ で UNIT-205/206 実装時に結線・拡張する
...
- [x] 未解決問題なし(本 v0.4 範囲、CI Pass 確認後に最終クローズ)
...
### UNIT-202(本 v0.4 で追加、CI 実績は Step 20 push 後追記)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass(Step 20 push 後追記)|
|------------|--------|--------------|------------|--------------------------------|
| UNIT-202 | UT-202-01〜02 | SDD §4.3(初期状態)| RCM-001 | (追記)|
| UNIT-202 | UT-202-03〜08 | SRS-002 / SDD §4.3 表(モード遷移正常系 6 件)| RCM-001 | (追記)|
| UNIT-202 | UT-202-09〜11 | SRS-002 / SDD §4.3(BeamState 事前条件)| **RCM-001 / HZ-001** | (追記)|
| UNIT-202 | UT-202-12〜17 | SRS-004 / SDD §4.3(エネルギー範囲)| **RCM-008 / HZ-005** | (追記)|
| UNIT-202 | **UT-202-18〜19** | SDD §4.3(API ガード)| **RCM-001 / HZ-001** | (追記。**Therac-25 Tyler 事故型「Electron API に X 線」「XRay API に Electron」を構造的拒否**)|
| UNIT-202 | UT-202-20 | SRS-D-002/003 / SDD §6.3(強い型 compile-time)| **RCM-008 / HZ-005** | (追記)|
| UNIT-202 | **UT-202-21** | SRS-D-009 / SDD §4.3 | **RCM-001 / RCM-002 / HZ-002** | **(`tsan` プリセットで 2 writer + 1 reader race 0 期待、追記)** |
| UNIT-202 | UT-202-22 | SDD §4.3(所有権)| — | (追記)|
| UNIT-202 | UT-202-23 | SDD §7 SOUP-003/004 | **RCM-002 / HZ-002 / HZ-007** | (追記)|
| UNIT-202 | UT-202-24 | SRS-003 / SDD §4.3(verify_mode_consistency)| RCM-001 / RCM-005(構造的前提)| (追記)|
| UNIT-202 | **UT-202-25** | **SDD §4.3 表 3 状態 × 3 要求モード = 9 組合せ網羅** | **RCM-001** | (追記。SDD §4.3 表との完全一致を機械検証)|
| UNIT-202 | UT-202-26 | SDD §4.1(防御的 enum 値)| RCM-001 | (追記)|
