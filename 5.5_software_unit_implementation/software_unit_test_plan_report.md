...
**バージョン:** 0.7
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2)/ 2026-04-30(v0.3〜v0.5)/ 2026-05-01(v0.6)/ 2026-05-01(v0.7 昇格)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005〜0010 + CR-0011(GitHub Issue #13、v0.7 UNIT-205 追加、HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)
...
>
> **本 v0.7(Step 23 / CR-0011)では UNIT-205 TurntableManager(HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3 + 偏差判定)を追加**。UNIT-206〜210 / 301〜304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| **UNIT-205** | **TurntableManager** | k-abe | `src/th25_ctrl/include/th25_ctrl/turntable_manager.hpp` + `src/turntable_manager.cpp` | **本 v0.7 で計画 + 実施記録(Step 23 / CR-0011、HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)** |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202 + UNIT-203 + UNIT-204 + UNIT-205)= 163 件**
...
| 実施期間 | 2026-05-01 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: 本 Step 23 本体コミット(SHA は push 後事後追記)|
| 試験環境(ローカル骨格ビルド)| macOS / AppleClang 15.0.0、CMake 3.28+、Ninja 1.11+、`TH25_BUILD_TESTS=OFF` で `static_assert(std::atomic<double>::is_always_lock_free)` を AppleClang 15 で検証成功(`th25_ctrl` 静的ライブラリ `[1/4]..[4/4]` 完全ビルド成功)。3 系統センサ + target を `std::atomic<double>` 4 個で並行アクセス保護、SDD §4.6 サンプルどおり `TurntablePosition` struct + `median()` + `has_discrepancy()` を実装、SRS-006 所定 3 位置(Electron=0.0/XRay=50.0/Light=-50.0 mm)を範囲定数で外部化 |
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_turntable_manager_tests` 25 件追加。GitHub Actions cpp-build ワークフロー(run id は push 後追記)|
| **CI 結果** | **全 9 ジョブ success ✅ 期待**(push 後 `gh run view` で確定後、SHA 追記事後処理で記録)。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-205-23(1 writer + 4 reader × 5000 イテレーション)race detection 0 で Pass を期待**(`std::atomic<double>` の release-acquire ordering の正しさが両コンパイラで機械検証成立)。**UT-205-22 で SDD §4.6 表との完全一致を 9 組合せ網羅機械検証成立**(SDD ↔ 実装の構造的乖離防止メカニズムを 6 ユニット目に拡大)。**UT-205-15/16 で Therac-25 Tyler 事故型「ターンテーブル位置不整合」+「単一センサ故障時の検出」の構造的予防(has_discrepancy + median)を機械検証成立**(D 主要因の Inc.1 中核 RCM-005 達成)|
...
| UNIT-205 | (llvm-cov 未計測、Step 24+ 整備予定) | 同上 | — | UT 設計上は公開 API 3 種 + 補助 API 2 種 + 静的純粋関数 3 種 + 3 系統センサ × expected 位置 = 9 組合せ網羅 + 1 writer + 4 reader 並行 + move_to/read 並行 を網羅(本書 §7.2 / §7.3)。SDD §4.6「move_to の 500 ms タイムアウト」は Step 24+ で UNIT-303/402 結線時に実装、IT-104 で実測予定 |
...
| (Step 23 / CR-0011 範囲)| (ローカル骨格ビルド成功、CI 全 9 ジョブ Pass は push 後事後確認)| Minor 以下 | — | 進行中(2026-05-01、SHA 追記事後処理コミットで確定予定。Step 22 教訓: PRB 起票運用継続)|
...
### UNIT-205(本 Step 23 / CR-0011、HZ-001 直接対応 + RCM-005 中核、CI 実績は push 後 SHA 追記事後処理で確定)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-205 | UT-205-01〜02 | SDD §4.6 / SRS-006(初期状態 / SRS 定数)| RCM-001 / RCM-005 | 全 9 ジョブ Pass 期待(2026-05-01、push 後 run id 確定)|
...
| UNIT-205 | **UT-205-23** | SRS-D-009 / SDD §4.6 / §9 SEP-003(1 writer + 4 reader × 5000) | **RCM-002 / RCM-004 / HZ-002** | **`tsan` プリセットで race 0 確認(push 後)** |
| UNIT-205 | UT-205-24 | SRS-D-009 / SDD §4.6(mover + reader 並行)| RCM-002 / HZ-002 | 同上(`tsan` で race-free 確認)|
| UNIT-205 | UT-205-25 | SDD §4.1.1 / §6.2(ErrorCode 階層整合 Turntable 系 0x04)| RCM-009 | 同上 |
