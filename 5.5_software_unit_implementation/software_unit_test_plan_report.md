...
**バージョン:** 0.8
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2)/ 2026-04-30(v0.3〜v0.5)/ 2026-05-01(v0.6〜v0.8)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005〜0011 + CR-0012(GitHub Issue #16、v0.8 UNIT-206 追加、HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)
...
>
> **本 v0.8(Step 24 / CR-0012)では UNIT-206 BendingMagnetManager(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表 + ±5% 偏差判定)を追加**。UNIT-207〜210 / 301〜304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| [11] | 構成アイテム一覧(CIL-TH25-001) | 0.16 |
| [12] | CCB 運用規程(CCB-TH25-001) | 0.1 |
| [13] | 変更要求台帳(CRR-TH25-001) | 0.14 |
...
| **UNIT-206** | **BendingMagnetManager** | k-abe | `src/th25_ctrl/include/th25_ctrl/bending_magnet_manager.hpp` + `src/bending_magnet_manager.cpp` | **本 v0.8 で計画 + 実施記録(Step 24 / CR-0012、HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)** |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202 + UNIT-203 + UNIT-204 + UNIT-205 + UNIT-206)= 194 件**
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_bending_magnet_manager_tests` 31 件追加。GitHub Actions cpp-build ワークフロー(run id は push 後追記)|
| **CI 結果** | (push 後追記)。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-206-28(1 setter + 4 reader × 5000 イテレーション)race detection 0 で Pass 予定**(`std::atomic<double>` × 2 + `std::atomic<bool>` の release-acquire ordering の正しさを両コンパイラで機械検証する設計)。**UT-206-27 で SDD §4.7 表との完全一致を 14 組合せ網羅機械検証成立予定**(SDD ↔ 実装の構造的乖離防止メカニズムを 7 ユニット目に拡大)。**UT-206-13/14 で EnergyMagnetMap 線形補間 (slope, intercept) を constexpr `static_assert` で機械検証成立予定**(SDD §4.7 で予告された「コードに直接埋め込まない、HZ-007 構造的予防」の実装層実現、SDP §6.1 確定後に校正データへ CR で置換可能な構造)。**UT-206-26 で Energy_MeV / Energy_MV / MagnetCurrent_A の compile-time 隔離を機械検証成立予定**(全 7 ペアで暗黙変換不可、HZ-005 直接対応の Inc.1 中核達成)|
...
| (Step 24 / CR-0012 範囲)| (ローカル骨格ビルド成功 `[1/4]..[4/4]`、CI 全 9 ジョブ Pass は push 後追記、本体コミット SHA は事後処理で記録)| Minor 以下 | — | (push 後最終確定) |
...
### UNIT-206(本 Step 24 / CR-0012、HZ-005 部分対応 + RCM-008 中核、CI 実績は push 後追記)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-206 | UT-206-01 | SDD §4.7(初期状態)| RCM-008 | (push 後追記)|
