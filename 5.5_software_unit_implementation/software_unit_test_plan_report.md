...
**バージョン:** 0.5
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2)/ 2026-04-30(v0.3)/ 2026-04-30(v0.4)/ 2026-04-30(v0.5 昇格)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005〜0008 + CR-0009(GitHub Issue #9、v0.5 UNIT-203 追加、RCM-017 中核)
...
>
> **本 v0.5(Step 21 / CR-0009)では UNIT-203 BeamController(RCM-017 中核、ビームオン許可フラグ消費型)を追加**。UNIT-204〜210 / 301〜304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| **UNIT-203** | **BeamController** | k-abe | `src/th25_ctrl/include/th25_ctrl/beam_controller.hpp` + `src/beam_controller.cpp` | **本 v0.5 で計画 + 実施記録(Step 21 / CR-0009、RCM-017 中核 / 許可フラグ消費型)** |
| UNIT-101〜104 | Operator UI 群 | k-abe | `src/th25_ui/...`(Inc.4 で追加予定) | (Inc.4 SDD 改訂時に追加) |
| UNIT-204 | DoseManager | k-abe | `src/th25_ctrl/...` | (Step 22+ 後続で追加予定) |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202 + UNIT-203)= 106 件**
...
| 実施期間 | 2026-04-30 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 21 push 後に SHA 追記事後処理で記録) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_beam_controller_tests` 19 件追加。GitHub Actions cpp-build ワークフロー(本 Step push 後に run id 確定)|
| **CI 結果** | (本 Step push 後に SHA 追記事後処理で確定。期待値: 全 9 ジョブ success ✅、`clang-17/tsan` + `gcc-13/tsan` で **UT-203-13(8 thread × 1000 セット並列 beam_on)が「各セットで唯一 1 thread のみ成功」+ race detection 0** で Pass、Therac-25 Tyler 事故型「許可確認の race condition」の `compare_exchange_strong` による消費型予防が機械検証成立、UT-203-07 で「ビーム二重照射」の構造的拒否を機械検証成立、UT-203-17 で setter/reader 並行 race-free 検証成立)|
...
| UNIT-202 | (llvm-cov 未計測、Step 22+ 整備予定) | 同上 | — | UT 設計上は公開 API 5 種 + モード遷移 9 組合せ網羅 + BeamState/エネルギー範囲ガード + 強い型 compile-time + 並行処理を網羅(本書 §7.2 / §7.3) |
| UNIT-203 | (llvm-cov 未計測、Step 22+ 整備予定) | 同上 | — | UT 設計上は公開 API 5 種 + 許可フラグ消費型 + Lifecycle/state ガード + 並行処理(8 thread × 1000 セット)を網羅(本書 §7.2 / §7.3)。SDD §4.4「< 10 ms ビームオフ」の実測は IT-101(Inc.1 完了時)で実施 |
...
| (Step 21 / CR-0009 範囲)| (本 Step push 後に CI 結果に応じて追記)| | | |
...
- **UNIT-203 と ElectronGunSim / InterProcessChannel の結線**: 本 v0.5 では UNIT-203 単体の状態機械 + 許可フラグ消費型管理を実装。SDD §4.4「< 10 ms 電子銃電流 0 mA 確認」+「ウォッチドッグ」+「InterProcessChannel 経由 ElectronGunCurrent_mA 設定」は Step 22+ で UNIT-301/402 実装時に結線、Inc.1 完了時の **IT-101(< 10 ms 実測)** で SRS-101 の妥当性確認を行う
...
### UNIT-203(v0.5、本 Step 21、RCM-017 中核 / 許可フラグ消費型)
...
- [x] 未解決問題なし(本 v0.5 範囲、CI Pass 確認後に最終クローズ)。**SDD §4.4 < 10 ms ビームオフ実測は IT-101(Inc.1 完了時)で実施予定**
...
### UNIT-203(本 v0.5 で追加、CI 実績は Step 21 push 後追記、RCM-017 中核)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass(Step 21 push 後追記)|
|------------|--------|--------------|------------|--------------------------------|
| UNIT-203 | UT-203-01〜03 | SDD §4.4 / §4.1(初期状態 / permission)| RCM-017 | (追記)|
| UNIT-203 | UT-203-04 | SRS-007 / SDD §4.4(Lifecycle ガード 7 状態)| **RCM-017 / HZ-001** | (追記)|
| UNIT-203 | UT-203-05 | SDD §4.4(permission ガード)| RCM-017 / HZ-004 | (追記)|
| UNIT-203 | UT-203-06 | SRS-007 / SDD §4.4(成功遷移)| RCM-017 | (追記)|
| UNIT-203 | **UT-203-07** | SDD §4.4(1 回限り消費)| **RCM-017 / HZ-001 / HZ-004** | (追記。**Therac-25 Tyler 事故型「ビーム二重照射」の構造的拒否を機械検証**)|
| UNIT-203 | UT-203-08〜09 | SRS-101 / SDD §4.4(beam_off 正常系)| RCM-017 | (追記)|
| UNIT-203 | UT-203-10 | SDD §4.4(再 beam_on は permission 再設定必要)| **RCM-017 / HZ-001** | (追記)|
| UNIT-203 | UT-203-11 | SDD §4.4(所有権)| — | (追記)|
| UNIT-203 | UT-203-12 | SDD §7 SOUP-003/004 | **RCM-002 / HZ-002 / HZ-007** | (追記)|
| UNIT-203 | **UT-203-13** | SRS-D-009 / SDD §4.4 | **RCM-002 / RCM-017 / HZ-002 / HZ-004** | **(`tsan` プリセットで 8 thread × 1000 セット race 0 + 各セット唯一 1 件成功 期待、追記)** |
| UNIT-203 | UT-203-14 | SRS-D-009 / SDD §4.4(並列 beam_off)| RCM-002 / RCM-017 | (追記)|
| UNIT-203 | UT-203-15 | SDD §4.1(BeamState 値安定性)| — | (追記)|
| UNIT-203 | UT-203-16 | SDD §4.4(防御的 Lifecycle 値)| RCM-017 | (追記)|
| UNIT-203 | UT-203-17 | SRS-D-009 / SDD §4.4(setter/reader 並行)| RCM-002 / RCM-017 / HZ-002 | (追記)|
| UNIT-203 | UT-203-18 | SDD §4.4(state ガード、再エントラント防止)| RCM-017 | (追記)|
| UNIT-203 | UT-203-19 | SDD §4.4(beam_off フェイルセーフ性)| RCM-017 | (追記)|
