...
| 実施期間 | 2026-05-02 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: TBD(本 Step 27 push 後の SHA 追記事後処理で確定) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_bending_magnet_sim_tests` 19 件追加。GitHub Actions cpp-build ワークフロー(run id: TBD、本 Step 27 push 後の SHA 追記事後処理で確定)+ Documentation Checks(run id: TBD) |
| **CI 結果** | **TBD**(本 Step 27 push 後の SHA 追記事後処理で確定予定。期待値: 全 9 ジョブ success ✅ + Documentation Checks success。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-302-19(1 setter + 4 reader × 5000 イテレーション)race detection 0 で Pass 予定**(`std::atomic<double>` + `bool` + `std::uint32_t` の release-acquire ordering の正しさが両コンパイラで機械検証成立、HZ-002 機械的予防を 11 ユニット目に拡大)。**UT-302-18 で SDD §4.13 表との完全一致を 7 ケース網羅機械検証成立予定**(SDD ↔ 実装の構造的乖離防止メカニズムを 10 ユニット目に拡大)。**UT-302-10/11 でノイズ ±2% 許容範囲 + seed 再現性を機械検証成立予定**(SDD §4.13「設定値 ± 2% のノイズ模擬」+ UT 駆動安定化 default OFF の構造的実現、UNIT-301 ±1% に対し UNIT-302 ±2% を機械検証で区別)。**UT-302-06/07 で範囲外指令の物理的飽和(clamp_to_range)を機械検証成立予定**(SRS-D-006 範囲外を物理デバイスの飽和挙動として Sim で模擬)|
...
| (Step 27 / CR-0015 範囲)| (ローカル骨格ビルド成功、`th25_sim` ライブラリに `bending_magnet_sim.cpp` を 3 cpp 目として組込成功、CI 全 9 ジョブ Pass + Documentation Checks success は push 後の SHA 追記事後処理で確定予定、PRB 起票なし予定)| TBD | — | **TBD(本 Step 27 push 後確定)** |
...
### UNIT-302(本 Step 27 / CR-0015、Hardware Simulator 層 2 ユニット目、CI 実績 push 後の SHA 追記事後処理で確定予定)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-302 | UT-302-01 | SDD §4.13(初期状態)| RCM-002 | TBD(本 Step 27 push 後確定予定)|
...
| UNIT-302 | **UT-302-18** | **SRS-IF-002 / SRS-O-002 / SRS-I-007 / SDD §4.13 表 7 ケース網羅** | **RCM-008** | 同上(SDD §4.13 表との完全一致を機械検証成立予定、10 ユニット目)|
| UNIT-302 | **UT-302-19** | SRS-D-009 / SDD §4.13 / §9 SEP-003(1 setter + 4 reader × 5000)| **RCM-002 / RCM-019 / HZ-002** | **`tsan` プリセットで race 0 確認予定(本 Step 27 push 後)** |
