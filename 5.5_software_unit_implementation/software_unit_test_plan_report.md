...
| 実施期間 | 2026-04-26 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| Step 17 本体コミット(SHA は本 Step SHA 追記事後処理時に確定)|
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、CMake + Ninja + vcpkg(ベースライン `c3867e714d...`)、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。GitHub Actions cpp-build ワークフロー |

### 8.2 試験ケース結果

| 試験 ID | ローカル骨格ビルド | CI cpp-build マトリクス(Step 17 push 後に確定) | 備考 |
|--------|------------------|------------------------------------|------|
| UT-200-01〜05(enum 系)| 構文 OK | (push 後に追記) | enum 値・系統判定 |
| UT-200-06〜10b(強い型)| 構文 OK + compile-time 検証成功 | (push 後に追記) | `static_assert` + `requires` 式で構造的検証 |
| UT-200-11〜17(PulseCount/Counter)| **`static_assert(is_always_lock_free)` 通過** | (push 後に追記、特に `tsan` プリセットでの UT-200-16 race-free 確認が中核) | HZ-002/003/007 構造的予防の機械検証 |
| UT-200-18〜20b(Result)| 構文 OK | (push 後に追記) | C++23 `std::expected` 互換 IF |

> **本表は CI cpp-build マトリクス全 8 ジョブの実施結果を Step 17 push 完了後の SHA 追記事後処理時に追記する。** `tsan` プリセットでの UT-200-16(4 スレッド × 10000 increment)が race condition 検出 0 で Pass することが、本 UTPR の中核成果物。
