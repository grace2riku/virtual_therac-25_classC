...
**バージョン:** 0.12
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2)/ 2026-04-30(v0.3〜v0.5)/ 2026-05-01(v0.6〜v0.10)/ 2026-05-02(v0.11)/ 2026-05-04(v0.12)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005〜0015 + CR-0016(GitHub Issue #20、v0.12 UNIT-303 追加、Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能)
...
>
> **本 v0.12(Step 28 / CR-0016)では UNIT-303 TurntableSim(Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能)を追加**。UNIT-207 / 209 / 210 / 304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| [11] | 構成アイテム一覧(CIL-TH25-001) | 0.20 |
| [12] | CCB 運用規程(CCB-TH25-001) | 0.1 |
| [13] | 変更要求台帳(CRR-TH25-001) | 0.18 |
...
| **UNIT-303** | **TurntableSim** | k-abe | `src/th25_sim/include/th25_sim/turntable_sim.hpp` + `src/turntable_sim.cpp` | **本 v0.12 で計画 + 実施記録(Step 28 / CR-0016、Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse)** |
...
| UNIT-304 | Hardware Simulator 残ユニット(IonChamberSim) | k-abe | `src/th25_sim/...` | 同上(Step 29+ で追加) |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202 + UNIT-203 + UNIT-204 + UNIT-205 + UNIT-206 + UNIT-208 + UNIT-301 + UNIT-302 + UNIT-303)= 285 件**
...
| 実施期間 | 2026-05-04 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 28 push 後の SHA 追記事後処理で記載) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_turntable_sim_tests` 24 件追加。GitHub Actions cpp-build ワークフロー(run id および最終コミット SHA は Step 28 push 後の SHA 追記事後処理で記載)+ Documentation Checks(同上)|
| **CI 結果** | **全 9 ジョブ success ✅** 予定(`clang-tidy (clang-17)`、`clang-17 / debug`、`clang-17 / asan`、`clang-17 / ubsan`、`clang-17 / tsan`、`gcc-13 / debug`、`gcc-13 / asan`、`gcc-13 / ubsan`、`gcc-13 / tsan`)+ Documentation Checks success 予定。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-303-23(1 commander + 4 reader × 5000 イテレーション、3 系統センサ並行読取)+ UT-303-24(fault injector + 3 reader × 5000、`std::atomic<FaultMode>` 並行操作)race detection 0 で Pass 予定**(`std::atomic<double>` + `bool` + `std::uint32_t` + **`std::atomic<FaultMode>` の四重 atomic** の release-acquire ordering の正しさが両コンパイラで機械検証成立、HZ-002 機械的予防を 12 ユニット目に拡大)。**UT-303-22 で SDD §4.12 表との完全一致を 7 ケース網羅機械検証成立予定**(FaultMode 4 種 × 代表的 commanded 値、SDD ↔ 実装の構造的乖離防止メカニズムを 11 ユニット目に拡大)。**UT-303-12/13/14 で故障注入 3 モード × 3 系統センサ = 9 通りの代表シナリオを機械検証成立予定**(StuckAt 凍結値返却 / Delay 前回値返却 / NoResponse 0.0 固定返却)、UT-303-15 で部分故障(Sensor0 のみ StuckAt、他 2 系統正常)= **Therac-25 Tyler 事故型「単一センサ故障」の試験ハーネス**、UT-303-16 で混在故障モード(Sensor0=StuckAt + Sensor1=Delay + Sensor2=NoResponse)を機械検証成立予定。**UT-303-10/11 でセンサノイズ ±0.1 mm 絶対値 + seed 再現性を機械検証成立予定**(SDD §4.12「実位置 ± 0.1 mm」+ UT 駆動安定化 default OFF の構造的実現、UNIT-301/302 の乗算ノイズに対し本ユニットは絶対値ノイズで物理仕様に整合)。**UT-303-06/07 で範囲外指令の物理的飽和(clamp_to_range)を機械検証成立予定**(SRS-D-007 範囲外を物理デバイスの飽和挙動として Sim で模擬)。**Step 22/23 と異なり PRB 起票なし予定**(Step 24〜27 に続いて 5 連続のクリーンコミット成立予定、Step 26 教訓「`mutable std::atomic` パターンの初版採用」+ Step 27 教訓「Sim 層 `static_assert` 多重表明の継続拡大」継続反映が奏功)|
...
| (Step 28 / CR-0016 範囲)| (ローカル骨格ビルド成功、`th25_sim` ライブラリに `turntable_sim.cpp` を 4 cpp 目として組込成功、`static_assert` 四重表明(`std::atomic<double>` + `bool` + `std::uint32_t` + `FaultMode`)通過、CI 全 9 ジョブ Pass + Documentation Checks success 予定、PRB 起票なし予定)| Minor 以下 | — | **解決予定(2026-05-04、SHA 追記事後処理で run id 確定)** |
...
### UNIT-303(本 Step 28 / CR-0016、Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能、CI 実績 SHA 追記事後処理で確定予定)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-303 | UT-303-01 | SDD §4.12(初期状態)| RCM-002 / RCM-005 | 全 9 ジョブ Pass 予定(2026-05-04)|
...
| UNIT-303 | **UT-303-23** | SRS-D-009 / SDD §4.12 / §9 SEP-003(1 commander + 4 reader × 5000、3 系統センサ並行)| **RCM-002 / RCM-019 / HZ-002** | **`tsan` プリセットで race 0 確認予定** |
| UNIT-303 | **UT-303-24** | SRS-D-009 / SDD §4.12 / §9 SEP-003(fault injector + 3 reader × 5000、`std::atomic<FaultMode>` 並行)| **RCM-002 / RCM-019 / HZ-002** | **`tsan` プリセットで race 0 確認予定** |
