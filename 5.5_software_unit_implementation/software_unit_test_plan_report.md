...
**バージョン:** 0.3
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2 昇格)/ 2026-04-30(v0.3 昇格)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005(GitHub Issue #5、初版 UNIT-200)/ CR-0006(GitHub Issue #6、v0.2 UNIT-401 追加)/ CR-0007(GitHub Issue #7、v0.3 UNIT-201 追加)
...
>
> **本 v0.3(Step 19 / CR-0007)では UNIT-201 SafetyCoreOrchestrator(`LifecycleState` 8 状態機械)を追加**。UNIT-202〜210 / 301〜304 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| [11] | 構成アイテム一覧(CIL-TH25-001) | 0.11 |
| [12] | CCB 運用規程(CCB-TH25-001) | 0.1 |
| [13] | 変更要求台帳(CRR-TH25-001) | 0.9 |
...
|------------|------|------|------------------|----------------|
| **UNIT-200** | **CommonTypes** | k-abe | `src/th25_ctrl/include/th25_ctrl/common_types.hpp` | **v0.1 で計画 + 実施記録(Step 17 / CR-0005)、v0.2/v0.3 で維持** |
| **UNIT-401** | **InProcessQueue** | k-abe | `src/th25_ctrl/include/th25_ctrl/in_process_queue.hpp` | **v0.2 で計画 + 実施記録(Step 18 / CR-0006)、v0.3 で維持** |
| **UNIT-201** | **SafetyCoreOrchestrator** | k-abe | `src/th25_ctrl/include/th25_ctrl/safety_core_orchestrator.hpp` + `src/safety_core_orchestrator.cpp` | **本 v0.3 で計画 + 実施記録(Step 19 / CR-0007)** |
| UNIT-101〜104 | Operator UI 群 | k-abe | `src/th25_ui/...`(Inc.4 で追加予定) | (Inc.4 SDD 改訂時に追加) |
| UNIT-202 | TreatmentModeManager | k-abe | `src/th25_ctrl/...` | (Step 20+ 後続で追加予定) |
| UNIT-203 | BeamController | k-abe | `src/th25_ctrl/...` | (Step 20+ 後続で追加予定) |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201)= 61 件**
...
| 実施期間 | 2026-04-30 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 19 push 後に SHA 追記事後処理で記録) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_safety_core_orchestrator_tests` 24 件追加。GitHub Actions cpp-build ワークフロー(本 Step push 後に run id 確定)|
| **CI 結果** | (本 Step push 後に SHA 追記事後処理で確定。期待値: 全 9 ジョブ success ✅、`clang-17/tsan` + `gcc-13/tsan` で UT-201-24(SPSC イベント配送 200 サイクル × 6 イベント並行)race detection 0、UT-201-21 で SDD §6.1 表との完全一致を 88 組合せ網羅試験で機械的検証成立) |
...
|--------|------------------|----------------------------|------|
| UT-201-01〜03(初期 / init_subsystems / idempotency)| 構文 OK | (push 後追記)| `compare_exchange_strong` で二重呼出を構造的に拒否 |
| UT-201-04(lock-free 表明)| **`static_assert(std::atomic<LifecycleState>::is_always_lock_free)` + `static_assert(std::atomic<bool>::is_always_lock_free)` 通過** | (push 後追記)| HZ-007 構造的予防の build 時 fail-stop |
| UT-201-05〜06(所有権 / shutdown signal-safe)| 構文 OK | (push 後追記)| コピー/ムーブ禁止、`shutdown` は state 変更せず flag のみ |
| UT-201-07(EventLoop 正常終了)| 構文 OK | (push 後追記)| Idle で shutdown → run_event_loop 戻り値 0 |
| UT-201-08〜17(状態遷移許可表 10 件、SDD §6.1 個別)| 構文 OK | (push 後追記)| 各論理遷移を `next_state` 単独で機械検証 |
| UT-201-18〜20(不正遷移 / 終端 / 緊急停止)| 構文 OK | (push 後追記)| **HZ-001 直接対応**(Idle→BeamOn 直接禁止)、Halted 終端、ShutdownRequested 任意状態許可 |
| **UT-201-21**(網羅試験 88 組合せ)| 構文 OK | (push 後追記)| **SDD §6.1 表との完全一致を機械検証**(許可 19 / 拒否 69)|
| UT-201-22〜23(EventLoop 統合 + 不正遷移時の Halted)| 構文 OK | (push 後追記)| キュー駆動の状態遷移、不正イベントは Halted + 戻り値 1 |
| **UT-201-24** | 構文 OK | (push 後追記。期待値: `clang-17/tsan` + `gcc-13/tsan` で UT-201-24(SPSC イベント配送 200 サイクル × 6 イベント)race detection 0)| **HZ-002 直接対応の機械検証**(SDD §9 SEP-003 + RCM-019 のオーケストレータ層拡大)|
| (clang-tidy 全ファイル)| — | (push 後追記)| MISRA C++ 2023 関連チェック完全 Pass を期待 |

> **本表の中核成果物達成(計画段階):** UT-201-21 の 88 組合せ網羅試験で SDD §6.1 状態遷移許可表を機械的に検証する設計を確立(SDD と実装の構造的乖離を防ぐ仕組み)。UT-201-24 の SPSC イベント配送 concurrent 試験で、UNIT-401 InProcessQueue 経由の状態機械駆動の race condition 検出能力を Inc.1 オーケストレータ層に拡大(SRMP §6.1 3 層検証の TSan 層、Tyler 第 1 例 race condition 類型の機械的予防)。UT-201-18 の不正遷移拒否(Idle→BeamOn 直接禁止)で、HZ-001(電子モード+X 線ターゲット非挿入)の構造的不可能化を状態機械層で実体化(RCM-001 中核)。
...
| UNIT-201 | (llvm-cov 未計測、Step 20+ 整備予定) | 同上 | — | UT 設計上は公開 API 4 種 + 状態遷移 88 組合せ網羅 + イベントループ統合 + 並行処理を網羅(本書 §7.2 / §7.3) |
...
| (Step 19 / CR-0007 範囲)| (本 Step push 後に CI 結果に応じて追記)| | | |

### 8.5 未達項目と処置

- **llvm-cov 網羅率計測**: 本 v0.3 範囲では未実施。Step 20+ 後続ステップで CI ワークフローに統合する(SOUP-017 として CIL §5 で正式登録予定)。本 v0.3 では UT 設計上の網羅性確認(本書 §7.2 試験ケース定義 + UT-201-21 の 88 組合せ網羅試験)で代替
- **モデル検査**: Inc.3(タスク同期)着手時に TLA+ / SPIN / Frama-C のいずれかを採用判断(SDD §10 未確定事項、SOUP-019 検討中として CIL §5 で記録済)
- **UNIT-201 と各 Manager の結線**: 本 v0.3 では UNIT-201 単体の状態機械中核を実装。LifecycleEvent → 各 Manager メッセージ型(SDD §5.1 IF-U-001〜007)への変換アダプタは Step 20+ で UNIT-202〜206 実装時に追加
...
- [x] 未解決問題なし(本 v0.3 範囲、CI Pass 確認後に最終クローズ)
...
### UNIT-201(本 v0.3 で追加、CI 実績は Step 19 push 後追記)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass(Step 19 push 後追記)|
|------------|--------|--------------|------------|--------------------------------|
| UNIT-201 | UT-201-01 | SDD §4.2 / §6.1(初期状態)| RCM-001 | (追記)|
| UNIT-201 | UT-201-02〜03 | SRS-012 / SDD §4.2(自己診断)| RCM-001 / RCM-013 | (追記)|
| UNIT-201 | UT-201-04 | SDD §7 SOUP-003/004 | **RCM-002 / HZ-002 / HZ-007** | (追記)|
| UNIT-201 | UT-201-05 | SDD §4.2(所有権)| — | (追記)|
| UNIT-201 | UT-201-06〜07 | SDD §4.2(`shutdown()` signal-safe / `run_event_loop` 戻り値)| RCM-017 | (追記)|
| UNIT-201 | UT-201-08〜17 | SDD §6.1 状態遷移許可表(10 件論理遷移)| RCM-001 / RCM-017 | (追記)|
| UNIT-201 | UT-201-18 | SDD §6.1(Idle→BeamOn 不正遷移)| **RCM-001 / HZ-001** | (追記)|
| UNIT-201 | UT-201-19〜20 | SDD §6.1(終端 Halted / 緊急停止 ShutdownRequested)| RCM-001 / RCM-017 | (追記)|
| UNIT-201 | **UT-201-21** | **SDD §6.1 表 8 状態 × 11 イベント = 88 組合せ網羅** | **RCM-001** | (追記。SDD と実装の完全一致を機械検証)|
| UNIT-201 | UT-201-22〜23 | SDD §4.2 / §6.1 / §4.13(EventLoop 統合)| RCM-001 / RCM-019 | (追記)|
| UNIT-201 | **UT-201-24** | SRS-D-009 / SDD §9 SEP-003 / §4.13 | **RCM-002 / RCM-019 / HZ-002** | **(`tsan` プリセットで SPSC イベント配送 race 0 期待、追記)** |
