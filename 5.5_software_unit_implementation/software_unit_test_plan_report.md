...
**バージョン:** 0.13
**作成日:** 2026-04-26(初版)/ 2026-04-29(v0.2)/ 2026-04-30(v0.3〜v0.5)/ 2026-05-01(v0.6〜v0.10)/ 2026-05-02(v0.11)/ 2026-05-04(v0.12〜v0.13)
**対象製品:** 仮想 Therac-25(Virtual Therac-25) / TH25-SIM-001
**対象ソフトウェアバージョン:** 0.2.0(Inc.1 完了で初回機能リリース予定)
**安全クラス:** C(IEC 62304)
**変更要求:** CR-0005〜0016 + CR-0017(GitHub Issue #21、v0.13 UNIT-304 追加、Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure)
...
>
> **本 v0.13(Step 29 / CR-0017)では UNIT-304 IonChamberSim(Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure)を追加 → Hardware Simulator 層(UNIT-301〜304)完成**。UNIT-207 / 209 / 210 / 402 の試験計画・実績は後続 SDD-準拠ステップで追加する。
...
| [11] | 構成アイテム一覧(CIL-TH25-001) | 0.21 |
| [12] | CCB 運用規程(CCB-TH25-001) | 0.1 |
| [13] | 変更要求台帳(CRR-TH25-001) | 0.19 |
...
**累積合計試験ケース(UNIT-200 + UNIT-401 + UNIT-201 + UNIT-202 + UNIT-203 + UNIT-204 + UNIT-205 + UNIT-206 + UNIT-208 + UNIT-301 + UNIT-302 + UNIT-303 + UNIT-304)= 309 件**
...
| 実施期間 | 2026-05-04 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 29 push 後の SHA 追記事後処理で記載) |
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_ion_chamber_sim_tests` 24 件追加。GitHub Actions cpp-build ワークフロー(run id および最終コミット SHA は Step 29 push 後の SHA 追記事後処理で記載)+ Documentation Checks(同上)|
| **CI 結果** | **全 9 ジョブ success ✅** 予定(`clang-tidy (clang-17)`、`clang-17 / debug`、`clang-17 / asan`、`clang-17 / ubsan`、`clang-17 / tsan`、`gcc-13 / debug`、`gcc-13 / asan`、`gcc-13 / ubsan`、`gcc-13 / tsan`)+ Documentation Checks success 予定。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-304-23(1 incrementer + 4 reader × 5000 イテレーション、両チャネル並行読取)+ UT-304-24(fault injector + 3 reader × 5000、`std::atomic<FaultMode>` 並行操作)race detection 0 で Pass 予定**(`std::atomic<double>` + **`std::atomic<FaultMode>` の二重 atomic** の release-acquire ordering の正しさが両コンパイラで機械検証成立、HZ-002 機械的予防を 13 ユニット目に拡大、コード実装範囲内全 13 ユニットに展開完了)。**UT-304-22 で SDD §4.12 表との完全一致を 6 ケース網羅機械検証成立予定**(FaultMode 3 種 × 代表的 dose 値、SDD ↔ 実装の構造的乖離防止メカニズムを 12 ユニット目に拡大)。**UT-304-11/12 で故障注入 Saturation/ChannelFailure × 2 チャネル機械検証成立予定** + **UT-304-13/14 で部分故障(Channel0/1 のみ Saturation/ChannelFailure)= Therac-25 ドーズ計測異常型「片系故障時の検出経路」+「片系応答消失」試験ハーネス機械検証成立予定** + **UT-304-15 で混在故障モード(Channel0=Saturation + Channel1=ChannelFailure)= Therac-25 ドーズ計測異常型「2 チャネル不一致」機械検証成立予定**。**UT-304-06/07/08 で範囲外指令の物理的飽和(clamp_to_range, SRS-I-006 0.0〜10000.0 cGy、上下限両方向)機械検証成立予定**。**Step 22/23 と異なり PRB 起票なし予定**(Step 24〜28 に続いて 6 連続のクリーンコミット成立予定、Step 26〜28 教訓「`mutable std::atomic` パターン + 多重 atomic 設計の初版採用 + Sim 層設計パターン踏襲」継続反映が奏功)|
...
| (Step 29 / CR-0017 範囲)| (ローカル骨格ビルド成功、`th25_sim` ライブラリに `ion_chamber_sim.cpp` を 5 cpp 目として組込成功 → **Hardware Simulator 層完成**、`static_assert` 二重表明(`std::atomic<double>` + `FaultMode`)通過、CI 全 9 ジョブ Pass + Documentation Checks success 予定、PRB 起票なし予定)| Minor 以下 | — | **解決予定(2026-05-04、SHA 追記事後処理で run id 確定)** |
...
### UNIT-304(本 Step 29 / CR-0017、Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure、CI 実績 SHA 追記事後処理で確定予定)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-304 | UT-304-01 | SDD §4.12(初期状態)| RCM-002 / RCM-003 | 全 9 ジョブ Pass 予定(2026-05-04)|
...
| UNIT-304 | **UT-304-23** | SRS-D-009 / SDD §4.12 / §9 SEP-003(1 incrementer + 4 reader × 5000、両チャネル並行読取)| **RCM-002 / RCM-019 / HZ-002** | **`tsan` プリセットで race 0 確認予定** |
| UNIT-304 | **UT-304-24** | SRS-D-009 / SDD §4.12 / §9 SEP-003(fault injector + 3 reader × 5000、`std::atomic<FaultMode>` 並行)| **RCM-002 / RCM-019 / HZ-002** | **`tsan` プリセットで race 0 確認予定** |
