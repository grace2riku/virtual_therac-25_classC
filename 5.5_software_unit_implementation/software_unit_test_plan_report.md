...
| 実施期間 | 2026-05-07 |
| 実施者 | k-abe |
| 対象ソフトウェアバージョン(コミット)| 本体: (Step 31 push 後追記)|
...
| 試験環境(CI 完全実行)| Ubuntu 24.04 + clang-17 / gcc-13、4 プリセット × 2 コンパイラ = 8 ジョブ + clang-tidy ジョブ。`th25_audit_logger_tests` 12 件追加。GitHub Actions cpp-build ワークフロー(Step 31 push 後の SHA 追記事後処理で記録予定)|
| **CI 結果** | **全 9 ジョブ success(予定、push 後の SHA 追記事後処理で確定)**(`clang-tidy (clang-17)`、`clang-17 / debug`、`clang-17 / asan`、`clang-17 / ubsan`、`clang-17 / tsan`、`gcc-13 / debug`、`gcc-13 / asan`、`gcc-13 / ubsan`、`gcc-13 / tsan`)+ Documentation Checks success。**特に `clang-17/tsan` + `gcc-13/tsan` で UT-209-09(4 publishers × 200 publish 並行)+ UT-209-10(1 io_runner + 2 publishers + 4 readers 並行)race detection 0 で Pass を期待**(`std::atomic<bool>` + `std::atomic<std::uint64_t>` の二重 atomic + `std::mutex` cerr 出力アトミック性 + release-acquire ordering の正しさが両コンパイラで機械検証成立、HZ-002 機械的予防を **15 ユニット目に拡大、Inc.1 範囲全 14 ユニット + 空殻ユニット第 2 例**)。**UT-209-12 で `static_assert(is_always_lock_free)` 二重表明を 15 ユニット目に拡大成立**(Inc.1 範囲全 14 ユニット + 空殻ユニット第 2 例)。**Step 24〜30 教訓継続反映:** (1) `static_assert(is_always_lock_free)` 統一(PRB-0001)、(2) UT 実装時に SDD §4.10 仕様セルフチェック(PRB-0002)、(3) 空殻範囲 atomic 最小化 + Sim 層設計パターン踏襲(Step 26〜30 教訓、初版即ビルド成功で PRB 起票回避)、(4) push 前ローカル markdownlint 実行運用継続。**Step 30 PRB-0005 教訓継続反映:** UT 並行試験の reader 設計で「setter loop が極端に高速で reader thread 起動前に完了するケース」回避のため、UT-209-09 では publish 反復回数を 200 に増やし、UT-209-10 では reader 反復回数を 1000 に設定 |
...
| (Step 31 / CR-0019 範囲)| (ローカル骨格ビルド成功、`th25_ctrl` ライブラリに `audit_logger.cpp` を 10 cpp 目として組込成功、`static_assert` 二重表明(`std::atomic<bool>` + `std::atomic<std::uint64_t>`)通過、CI 全 9 ジョブ Pass 予定 + Documentation Checks success 予定、Step 31 push 後の SHA 追記事後処理で確定予定、Step 30 PRB-0005 教訓継続反映により UT 並行試験で reader 反復回数を増やし flaky 回避)| Minor 以下 | — | **解決見込み(2026-05-07、run id は Step 31 push 後追記)** |
...
### UNIT-209(本 Step 31 / CR-0019、Inc.4 で本格化予定の安全関連イベントの集約・永続化ユニット 空殻実装、空殻実装としては第 2 例、CI 実績は push 後追記)

| ユニット ID | 試験 ID | 関連 SRS / SDD | 関連 RCM / HZ | CI ジョブ Pass |
|------------|--------|--------------|------------|--------------|
| UNIT-209 | UT-209-01 | SDD §4.10(初期状態)| RCM-002 | 全 9 ジョブ Pass(push 後追記)|
...
| UNIT-209 | **UT-209-09** | **SRS-D-009 / SDD §4.10 / §9 SEP-003(4 publishers × 200 publish 並行、std::mutex の cerr 出力アトミック性 + processed_count atomic 整合性)**| **RCM-002 / RCM-019 / HZ-002 / HZ-006** | **`tsan` プリセットで race 0 確認 push 後追記** |
| UNIT-209 | **UT-209-10** | **SRS-D-009 / SDD §4.10 / §9 SEP-003(1 io_runner + 2 publishers + 4 readers × 1000 並行、is_stop_requested + processed_count + pending_count 並行)**| **RCM-002 / RCM-019 / HZ-002 / HZ-006** | **`tsan` プリセットで race 0 確認 push 後追記** |
