...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 28 完了時点(UNIT-303 TurntableSim 実装 + UTPR v0.12、CR-0016 / GitHub Issue #20、2026-05-04、Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜27)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立、Step 24/25/26/27/28 では Step 23 教訓の継続反映により **PRB 起票なし**(Step 28 では Step 26/27 で確立した `mutable std::atomic` パターン + 三重 atomic 設計の初版採用 + Sim 層設計パターン踏襲によりローカルビルドエラー発生せず、5 連続クリーンコミット成立予定)。Step 29+(Hardware Simulator 残ユニット UNIT-304 + IPC 順次実装)以降は SDD §3 の残ユニット(UNIT-207 / 209 / 210 / 304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
| CI-SRC-005 | 仮想ハードウェアモデル | `src/th25_sim/` | 0.2.0(**Step 14 プレースホルダ → Step 26 で UNIT-301 ElectronGunSim → Step 27 で UNIT-302 BendingMagnetSim → Step 28 で UNIT-303 TurntableSim 実装段階に拡大(Hardware Simulator 層 3 ユニット目 / 故障注入機能あり)**)| C | `src/sim_stub.cpp`(Step 14 骨格)+ `include/th25_sim/electron_gun_sim.hpp` + `src/electron_gun_sim.cpp`(UNIT-301、Step 26)+ `include/th25_sim/bending_magnet_sim.hpp` + `src/bending_magnet_sim.cpp`(UNIT-302、Step 27)+ **`include/th25_sim/turntable_sim.hpp` + `src/turntable_sim.cpp`(UNIT-303、Step 28)**: SDD §4.12 仮想ターンテーブル応答模擬 + 故障注入機能、SRS-D-007 範囲(-100.0〜+100.0 mm)+ センサノイズ ±0.1 mm 絶対値(SDD §4.12「実位置 ± 0.1 mm」)+ 3 系統センサ数(`kSensorCount=3`)を範囲定数で外部化 + 公開 API 3 種(command_position / read_sensor / inject_fault、SRS-IF-003 完全一致)+ 補助 API 6 種(inject_stuck_at_value / enable_sensor_noise / disable_sensor_noise / is_sensor_noise_enabled / current_commanded / current_fault_mode、UT 駆動 + IT/ST 故障注入用)+ 静的純粋関数 3 種(is_position_in_range / clamp_to_range / sensor_index)、`enum class SensorId : std::uint8_t` + `enum class FaultMode : std::uint8_t`(None/StuckAt/Delay/NoResponse)を Sim 層スコープで新規定義(`common_types.hpp` 不変、`inc1-design-frozen` 維持)、`std::atomic<double>` × 5(commanded + previous + 3 系統 stuck_at)+ `std::atomic<FaultMode>` × 3(各センサ故障モード)+ `std::atomic<bool>`(noise_enabled)+ `mutable std::atomic<std::uint32_t>`(rng_state、簡易 LCG)の **計 10 atomic 変数 / 四重 atomic 種** で並行アクセス保護(UNIT-301/302 と同パターンを踏襲、Sim 層設計の一貫性確保)、`static_assert(is_always_lock_free)` **四重表明**(`std::atomic<FaultMode>` 追加)を **12 ユニット目に拡大**。`th25_sim` ライブラリの 4 cpp 目として組込(sim_stub + electron_gun_sim + bending_magnet_sim + turntable_sim)。UNIT-304 IonChamberSim は Step 29+ で SDD-TH25-001 v0.1.1 §4.12 に基づき追加。SAD(Step 15)で「仮想ハードウェアアーキテクチャ」として明示分離 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.12 | ドラフト(セルフ承認、Step 17〜27 で v0.1〜v0.11、**v0.12 昇格 Step 28 / CR-0016** で UNIT-303 TurntableSim 試験ケース 24 件追加(累積 285 件、Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1duodecies / §8.3 カバレッジ / §8.4 不具合・逸脱 / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-303 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.20 | ドラフト(セルフ承認、Step 28 反映で v0.19 → v0.20 昇格 — **CI-SRC-005 を UNIT-301 + UNIT-302 から UNIT-301 + UNIT-302 + UNIT-303(Hardware Simulator 層 3 ユニット目 / 故障注入機能あり)に拡大**、CI-DOC-UTPR を v0.11 → v0.12(UNIT-303 試験 24 件追加、累積 285 件)に、CI-DOC-CRR を v0.17 → v0.18、CI-DOC-DEVSTEPS を v0.26 → v0.27 に整合更新) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.27 | 継続更新(Step 0〜28 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27、v0.27 で Step 28(UNIT-303 TurntableSim / Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse)に対応) |
