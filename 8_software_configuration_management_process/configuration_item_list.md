...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 30 完了時点(UNIT-207 SafetyMonitor 空殻実装 + UTPR v0.14、CR-0018 / GitHub Issue #22、2026-05-07、Inc.2 で本格化予定の独立並行タスクのインターロック監視ユニット、空殻実装としては第 1 例)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜29)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立、Step 24/25/26/27/28/29/30 では Step 23 教訓の継続反映により **PRB 起票なし**(Step 30 では Step 26〜29 で確立した Sim 層設計パターン + 多重 atomic 設計の初版採用 + 空殻範囲 atomic 最小化の継続反映によりローカルビルドエラー発生せず、7 連続クリーンコミット成立予定)。Step 31+(Inc.2/Inc.4 該当の残空殻ユニット UNIT-209 AuditLogger / UNIT-210 CoreAuthenticationGateway + UNIT-402 InterProcessChannel 実装)以降は SDD §3 の残ユニット(UNIT-209 / 210 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.22 | ドラフト(セルフ承認、Step 30 反映で v0.21 → v0.22 昇格 — **CI-SRC-001 を UNIT-200/401/201/202/203/204/205/206/208 から UNIT-200/401/201/202/203/204/205/206/208 + UNIT-207 SafetyMonitor 空殻(Inc.2 で本格化予定 / 独立並行タスクのインターロック監視 / 空殻実装としては第 1 例)に拡大**、CI-DOC-UTPR を v0.13 → v0.14(UNIT-207 試験 12 件追加、累積 321 件)に、CI-DOC-CRR を v0.19 → v0.20、CI-DOC-DEVSTEPS を v0.28 → v0.29 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.20 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜30 で CR-0001〜0018 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設、CR-0018 は Inc.2 で本格化予定の UNIT-207 SafetyMonitor 空殻実装第 1 例) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.29 | 継続更新(Step 0〜30 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27、v0.27 で Step 28、v0.28 で Step 29(UNIT-304 IonChamberSim / Hardware Simulator 層完成)、v0.29 で Step 30(UNIT-207 SafetyMonitor 空殻 / Inc.2 で本格化予定の独立並行タスクのインターロック監視 / 空殻実装としては第 1 例)に対応) |
