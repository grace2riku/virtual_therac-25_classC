...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 24 完了時点(UNIT-206 BendingMagnetManager 実装 + UTPR v0.8、CR-0012 / GitHub Issue #16、2026-05-01、HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜23)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立。Step 24+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-207〜210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203(RCM-017 中核)、Step 22 で UNIT-204(HZ-005 直接 + HZ-003 構造的排除の中核)、Step 23 で UNIT-205(HZ-001 直接 + RCM-005 中核 / 3 系統センサ集約 med-of-3)、Step 24 で UNIT-206 BendingMagnetManager(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)実装段階に到達**。UNIT-200/401/201/202/203/204/205 に加え、`include/th25_ctrl/bending_magnet_manager.hpp` + `src/bending_magnet_manager.cpp`(UNIT-206): SRS-D-006 範囲(0.0〜500.0 A)+ SRS-ALM-004 偏差閾値(±5% = 0.05)を範囲定数で外部化 + `class EnergyMagnetMap`(線形補間係数 slope/intercept を constexpr で内部保持、electron: 2.5 A/MeV / xray: 10.0 A/MV、SDP §6.1 確定後の校正データに CR で置換可能)+ 公開 API 3 種(set_current_for_energy(Electron/XRay 2 オーバーロード)/ current_actual / is_within_tolerance)+ 補助 API 3 種(inject_actual_current / current_target / is_target_set)+ 静的純粋関数 4 種(EnergyMagnetMap::compute_target_current_electron/xray + is_current_within_tolerance + is_current_in_range)、`std::atomic<double>` 2 個(target_current_/actual_current_)+ `std::atomic<bool>`(target_set_)で並行保護、`static_assert(std::atomic<double>::is_always_lock_free)` + `static_assert(std::atomic<bool>::is_always_lock_free)` 二重表明を **8 ユニット目に拡大**(コード実装範囲内全 8 ユニットに展開完了)。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ `[1/4]..[4/4]` 完全ビルド成功)) | C | UNIT-207〜210 / 301〜304 / 402 は Step 25+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.8 | ドラフト(セルフ承認、Step 17〜23 で v0.1〜v0.7、**v0.8 昇格 Step 24 / CR-0012** で UNIT-206 BendingMagnetManager 試験ケース 31 件追加(累積 194 件、HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1octies / §8.3 カバレッジ / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-206 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.16 | ドラフト(セルフ承認、Step 24 反映で v0.15 → v0.16 昇格 — CI-SRC-001 を UNIT-200〜UNIT-206 実装段階(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)に、CI-DOC-UTPR を v0.7 → v0.8(UNIT-206 試験 31 件追加、累積 194 件)に、CI-DOC-CRR を v0.13 → v0.14、CI-DOC-DEVSTEPS を v0.22 → v0.23 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.14 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜24 で CR-0001〜0012 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.23 | 継続更新(Step 0〜24 を時系列記録。v0.7〜v0.22 で Step 9〜23、v0.23 で Step 24(UNIT-206 BendingMagnetManager / HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)に対応) |
