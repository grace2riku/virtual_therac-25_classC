...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 17 完了時点(UNIT-200 CommonTypes 実装 + UTPR-TH25-001 v0.1、CR-0005 / GitHub Issue #5、2026-04-26)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13: SRS + `inc1-requirements-frozen`、Step 14: SOUP 正式登録 + C++20 骨格、Step 15: SAD、Step 16: SDD + `inc1-design-frozen`、Step 17: UNIT-200 実装 + UTPR v0.1)に応じて昇格してきた。Step 18+(他ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-201〜210 / 301〜304 / 401〜402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(Step 17 で UNIT-200 CommonTypes 実装段階に到達。`include/th25_ctrl/common_types.hpp` ヘッダオンリー: 強い型 6 種 + `enum class` 5 種(TreatmentMode/LifecycleState/BeamState/ErrorCode/Severity)+ `Result<T,E>` + `PulseCounter` lock-free atomic、ローカル骨格ビルドで `static_assert(is_always_lock_free)` 検証成功) | C | UNIT-201〜210 は Step 18+ で SDD-TH25-001 v0.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.1 | ドラフト(セルフ承認、Step 17 / CR-0005 で作成。第 I 部 計画: §3 実装ルール + §4 検証プロセス(TSan + 境界値 + モデル検査の 3 層検証 SRMP §6.1 整合)+ §5 受入基準 5 項目 + §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目 + §7 UNIT-200 試験ケース 24 件を確定。第 II 部 報告: ローカル骨格ビルドで `static_assert(is_always_lock_free)` 検証成功、CI cpp-build マトリクス全 8 ジョブ Pass は Step 17 push 後の SHA 追記事後処理で記録予定) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.9 | ドラフト(セルフ承認、Step 17 反映で v0.8 → v0.9 昇格 — CI-SRC-001 を UNIT-200 実装段階に、CI-DOC-UTPR を「未着手」→ v0.1 に、CI-CFG-010 を Step 17 微調整版に、CI-DOC-CRR を v0.6 → v0.7、CI-DOC-DEVSTEPS を v0.15 → v0.16 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.7 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13/14/15/16/17 で CR-0001〜0005 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.16 | 継続更新(Step 0〜17 を時系列記録。v0.7 Step 9、v0.8 Step 10、v0.9 Step 11、v0.10 Step 12、v0.11 Step 13、v0.12 Step 14、v0.13 Step 14 SHA 追記事後処理、v0.14 Step 15、v0.15 Step 16、v0.16 Step 17 に対応) |
