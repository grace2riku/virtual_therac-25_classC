...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 27 完了時点(UNIT-302 BendingMagnetSim 実装 + UTPR v0.11、CR-0015 / GitHub Issue #19、2026-05-02、Hardware Simulator 層 2 ユニット目 / 仮想ベンディング磁石応答模擬)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜26)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立、Step 24/25/26/27 では Step 23 教訓の継続反映により **PRB 起票なし**(Step 27 では Step 26 で確立した `mutable std::atomic` パターンの初版採用によりローカルビルドエラー発生せず)。Step 28+(Hardware Simulator 残ユニット + IPC 順次実装)以降は SDD §3 の残ユニット(UNIT-207 / 209 / 210 / 303 / 304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
| CI-SRC-005 | 仮想ハードウェアモデル | `src/th25_sim/` | 0.2.0(**Step 14 プレースホルダ → Step 26 で UNIT-301 ElectronGunSim → Step 27 で UNIT-302 BendingMagnetSim 実装段階に拡大(Hardware Simulator 層 2 ユニット目)**)| C | `src/sim_stub.cpp`(Step 14 骨格)+ `include/th25_sim/electron_gun_sim.hpp` + `src/electron_gun_sim.cpp`(UNIT-301、Step 26)+ **`include/th25_sim/bending_magnet_sim.hpp` + `src/bending_magnet_sim.cpp`(UNIT-302、Step 27)**: SDD §4.13 仮想ベンディング磁石応答模擬、SRS-D-006 範囲(0.0〜500.0 A)+ ノイズ許容率(±2%、UNIT-301 ±1% に対し ±2%)を範囲定数で外部化 + 公開 API 2 種(set_current / read_actual_current、SRS-IF-002 完全一致)+ 補助 API 4 種(enable_noise / disable_noise / is_noise_enabled / current_commanded、UT 駆動)+ 静的純粋関数 2 種(is_current_in_range / clamp_to_range)、`std::atomic<double>`(commanded)+ `std::atomic<bool>`(noise_enabled)+ `mutable std::atomic<std::uint32_t>`(rng_state、簡易 LCG)の三重 atomic で並行アクセス保護(UNIT-301 と同パターンを踏襲、Sim 層設計の一貫性確保)、`static_assert(is_always_lock_free)` 三重表明を **11 ユニット目に拡大**。`th25_sim` ライブラリの 3 cpp 目として組込(sim_stub + electron_gun_sim + bending_magnet_sim)。UNIT-303 TurntableSim / UNIT-304 IonChamberSim は Step 28+ で SDD-TH25-001 v0.1.1 §4.12〜§4.13 に基づき順次追加。SAD(Step 15)で「仮想ハードウェアアーキテクチャ」として明示分離 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.11 | ドラフト(セルフ承認、Step 17〜26 で v0.1〜v0.10、**v0.11 昇格 Step 27 / CR-0015** で UNIT-302 BendingMagnetSim 試験ケース 19 件追加(累積 261 件、Hardware Simulator 層 2 ユニット目 / 仮想ベンディング磁石応答模擬)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1undecies / §8.3 カバレッジ / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-302 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.19 | ドラフト(セルフ承認、Step 27 反映で v0.18 → v0.19 昇格 — **CI-SRC-005 を UNIT-301 のみから UNIT-301 + UNIT-302(Hardware Simulator 層 2 ユニット目)に拡大**、CI-DOC-UTPR を v0.10 → v0.11(UNIT-302 試験 19 件追加、累積 261 件)に、CI-DOC-CRR を v0.16 → v0.17、CI-DOC-DEVSTEPS を v0.25 → v0.26 に整合更新) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.26 | 継続更新(Step 0〜27 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27(UNIT-302 BendingMagnetSim / Hardware Simulator 層 2 ユニット目 / 仮想ベンディング磁石応答模擬)に対応) |
