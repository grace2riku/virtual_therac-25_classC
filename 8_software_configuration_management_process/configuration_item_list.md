...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.24 | ドラフト(セルフ承認、Step 31 / CR-0019 で v0.23 → v0.24 昇格 — CI-SRC-001 を UNIT-209 AuditLogger 空殻実装段階に昇格、CI-DOC-UTPR を v0.14 → v0.15、CI-DOC-CRR を v0.21 → v0.22(CR-0019 登録)、CI-DOC-DEVSTEPS を v0.30 → v0.31 に整合更新、空殻実装としては第 2 例) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.22 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜31 で CR-0001〜0019 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設、CR-0019(UNIT-209 AuditLogger 空殻 / 空殻実装第 2 例)登録) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.31 | 継続更新(Step 0〜31 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27、v0.27 で Step 28、v0.28 で Step 29(UNIT-304 IonChamberSim / Hardware Simulator 層完成)、v0.29 で Step 30(UNIT-207 SafetyMonitor 空殻 / 空殻実装としては第 1 例)、v0.30 で Step 30 SHA 追記事後処理 + PRB-0005 反映(SPRP §3.1 PRB 起票プロセス本格運用第 3 例)、**v0.31 で Step 31(UNIT-209 AuditLogger 空殻 / Inc.4 で本格化予定の安全関連イベントの集約・永続化ユニット / 空殻実装としては第 2 例)に対応**) |
