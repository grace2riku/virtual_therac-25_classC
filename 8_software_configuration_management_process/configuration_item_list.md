...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 23 完了時点(UNIT-205 TurntableManager 実装 + UTPR v0.7、CR-0011 / GitHub Issue #13、2026-05-01、HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜22)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)が成立。Step 23+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-206〜210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203(RCM-017 中核)、Step 22 で UNIT-204(HZ-005 直接 + HZ-003 構造的排除の中核)、Step 23 で UNIT-205 TurntableManager(HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)実装段階に到達**。UNIT-200/401/201/202/203/204 に加え、`include/th25_ctrl/turntable_manager.hpp` + `src/turntable_manager.cpp`(UNIT-205): SRS-006 所定 3 位置(Electron=0.0/XRay=50.0/Light=-50.0 mm)+ SRS-D-007 範囲(-100.0〜+100.0 mm)+ SRS-ALM-003 偏差閾値(1.0 mm)+ SRS-006 in-position 許容偏差(±0.5 mm)を範囲定数で外部化 + `TurntablePosition` 構造体(sensor_0/1/2 + median + has_discrepancy)+ 公開 API 3 種(move_to/read_position/is_in_position)+ 補助 API 2 種(inject_sensor_readings/current_target)+ 静的純粋関数 3 種(compute_median/has_discrepancy_values/is_position_in_range)、`std::atomic<double>` 4 個(sensor_0/1/2 + target)で並行保護、`static_assert(std::atomic<double>::is_always_lock_free)` 表明を 7 ユニット目に拡大。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ `[1/4]..[4/4]` 完全ビルド成功)) | C | UNIT-206〜210 / 301〜304 / 402 は Step 24+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.7 | ドラフト(セルフ承認、Step 17〜22 で v0.1〜v0.6、**v0.7 昇格 Step 23 / CR-0011** で UNIT-205 TurntableManager 試験ケース 25 件追加(累積 163 件、HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1septies / §8.2 試験ケース結果 / §9 結論 / §10 トレーサビリティ表に UNIT-205 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.15 | ドラフト(セルフ承認、Step 23 反映で v0.14 → v0.15 昇格 — CI-SRC-001 を UNIT-200〜UNIT-205 実装段階(HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3)に、CI-DOC-UTPR を v0.6 → v0.7(UNIT-205 試験 25 件追加、累積 163 件)に、CI-DOC-CRR を v0.12 → v0.13、CI-DOC-DEVSTEPS を v0.21 → v0.22 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.13 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜23 で CR-0001〜0011 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.22 | 継続更新(Step 0〜23 を時系列記録。v0.7〜v0.21 で Step 9〜22、v0.22 で Step 23(UNIT-205 TurntableManager / HZ-001 直接 + RCM-005 中核)に対応) |
