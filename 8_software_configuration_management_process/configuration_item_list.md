...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 25 完了時点(UNIT-208 StartupSelfCheck 実装 + UTPR v0.9、CR-0013 / GitHub Issue #17、2026-05-01、RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜24)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立、Step 24/25 では Step 23 教訓の継続反映により **PRB 起票なし**。Step 25+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-207 / 209 / 210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.9 | ドラフト(セルフ承認、Step 17〜24 で v0.1〜v0.8、**v0.9 昇格 Step 25 / CR-0013** で UNIT-208 StartupSelfCheck 試験ケース 29 件追加(累積 223 件、RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1nonies / §8.3 カバレッジ / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-208 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.17 | ドラフト(セルフ承認、Step 25 反映で v0.16 → v0.17 昇格 — CI-SRC-001 を UNIT-200〜UNIT-208 実装段階(RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)に、CI-DOC-UTPR を v0.8 → v0.9(UNIT-208 試験 29 件追加、累積 223 件)に、CI-DOC-CRR を v0.14 → v0.15、CI-DOC-DEVSTEPS を v0.23 → v0.24 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.15 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜25 で CR-0001〜0013 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.24 | 継続更新(Step 0〜25 を時系列記録。v0.7〜v0.23 で Step 9〜24、v0.24 で Step 25(UNIT-208 StartupSelfCheck / RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)に対応) |
