...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 18 完了時点(UNIT-401 InProcessQueue 実装 + SDD v0.1.1 軽微修正 + UTPR v0.2、CR-0006 / GitHub Issue #6、2026-04-29)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13: SRS + `inc1-requirements-frozen`、Step 14: SOUP 正式登録 + C++20 骨格、Step 15: SAD、Step 16: SDD + `inc1-design-frozen`、Step 17: UNIT-200 実装 + UTPR v0.1、Step 18: UNIT-401 SPSC lock-free queue 実装 + SDD v0.1.1 + UTPR v0.2)に応じて昇格してきた。Step 19+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-201〜210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200 CommonTypes、Step 18 で UNIT-401 InProcessQueue 実装段階に到達**。`include/th25_ctrl/common_types.hpp`(UNIT-200): 強い型 6 種 + `enum class` 5 種 + `Result<T,E>` + `PulseCounter` lock-free atomic / `include/th25_ctrl/in_process_queue.hpp`(UNIT-401): 独自 SPSC lock-free ring buffer(Lamport 1983)、template `InProcessQueue<T, Capacity>`、`std::atomic<size_t>` head/tail + power-of-two Capacity 制約。ローカル骨格ビルドで両ヘッダの `static_assert` 全件通過(AppleClang 15)) | C | UNIT-201〜210 は Step 19+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.2 | ドラフト(セルフ承認、初版 Step 17 / CR-0005、**v0.2 昇格 Step 18 / CR-0006** で UNIT-401 InProcessQueue 試験ケース 13 件追加(累積 37 件)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1bis / §8.2 試験ケース結果 / §9 結論 / §10 トレーサビリティ表に UNIT-401 セクション追加。SDD-TH25-001 v0.1.1 への参照に更新) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.10 | ドラフト(セルフ承認、Step 18 反映で v0.9 → v0.10 昇格 — CI-SRC-001 を UNIT-200+UNIT-401 実装段階に、CI-DOC-SDD を v0.1 → v0.1.1(軽微改訂)に、CI-DOC-UTPR を v0.1 → v0.2(UNIT-401 試験 13 件追加)に、CI-DOC-CRR を v0.7 → v0.8、CI-DOC-DEVSTEPS を v0.16 → v0.17 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.8 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13/14/15/16/17/18 で CR-0001〜0006 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.17 | 継続更新(Step 0〜18 を時系列記録。v0.7 Step 9、v0.8 Step 10、v0.9 Step 11、v0.10 Step 12、v0.11 Step 13、v0.12 Step 14、v0.13 Step 14 SHA 追記事後処理、v0.14 Step 15、v0.15 Step 16、v0.16 Step 17、v0.17 Step 18 に対応) |
