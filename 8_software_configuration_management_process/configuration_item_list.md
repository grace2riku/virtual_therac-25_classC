...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 22 完了時点(UNIT-204 DoseManager 実装 + UTPR v0.6、CR-0010 / GitHub Issue #10、2026-05-01、HZ-005 直接 + HZ-003 構造的排除の中核)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13: SRS + `inc1-requirements-frozen`、Step 14: SOUP 正式登録 + C++20 骨格、Step 15: SAD、Step 16: SDD + `inc1-design-frozen`、Step 17: UNIT-200 実装 + UTPR v0.1、Step 18: UNIT-401 SPSC lock-free queue 実装 + SDD v0.1.1 + UTPR v0.2、Step 19: UNIT-201 SafetyCoreOrchestrator 実装 + UTPR v0.3、Step 20: UNIT-202 TreatmentModeManager 実装 + UTPR v0.4、Step 21: UNIT-203 BeamController 実装 + UTPR v0.5)に応じて昇格してきた。Step 22+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-205〜210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203(RCM-017 中核 / 許可フラグ消費型)、Step 22 で UNIT-204 DoseManager(HZ-005 直接 + HZ-003 構造的排除の中核)実装段階に到達**。UNIT-200/401/201/202/203 に加え、`include/th25_ctrl/dose_manager.hpp` + `src/dose_manager.cpp`(UNIT-204): SDD §6.5 で予告された強い型 `DoseRatePerPulse_cGy_per_pulse` を本ヘッダで導入 + `PulseCounter`(UNIT-200)を内包しドーズ積算 + `std::atomic<uint64_t> target_pulses_`(整数比較化、is_always_lock_free 保証)+ `std::atomic<bool> target_reached_`/`target_set_`(observer pattern フラグ、UNIT-201/UNIT-203 が後続 Step で観測)+ 公開 API 3 種(set_dose_target/on_dose_pulse/current_accumulated)+ 補助 API 5 種(current_target/is_target_reached/pulse_count_to_dose/reset/target_pulses_for_test)+ 静的純粋関数(is_dose_target_in_range)、`std::atomic<std::uint64_t>` + `std::atomic<bool>` 二重 `is_always_lock_free` `static_assert` 表明。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ `[1/4]..[4/4]` 完全ビルド成功)) | C | UNIT-205〜210 / 301〜304 / 402 は Step 23+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.6 | ドラフト(セルフ承認、Step 17〜21 で v0.1〜v0.5、**v0.6 昇格 Step 22 / CR-0010** で UNIT-204 DoseManager 試験ケース 32 件追加(累積 138 件、HZ-005 直接 + HZ-003 構造的排除の中核)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1sexies / §8.2 試験ケース結果 / §9 結論 / §10 トレーサビリティ表に UNIT-204 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.14 | ドラフト(セルフ承認、Step 22 反映で v0.13 → v0.14 昇格 — CI-SRC-001 を UNIT-200〜UNIT-204 実装段階(HZ-005 直接 + HZ-003 構造的排除の中核)に、CI-DOC-UTPR を v0.5 → v0.6(UNIT-204 試験 32 件追加、累積 138 件)に、CI-DOC-CRR を v0.11 → v0.12、CI-DOC-DEVSTEPS を v0.20 → v0.21 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.12 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜22 で CR-0001〜0010 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.21 | 継続更新(Step 0〜22 を時系列記録。v0.7〜v0.20 で Step 9〜21、v0.21 で Step 22(UNIT-204 DoseManager / HZ-005 直接 + HZ-003 構造的排除の中核)に対応) |
