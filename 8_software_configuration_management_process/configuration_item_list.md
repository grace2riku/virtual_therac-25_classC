...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)着手 + プロジェクト骨格構築完了時点(Step 14、CR-0002 / GitHub Issue #2、2026-04-26)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 着手(Step 13: SRS 追加、`inc1-requirements-frozen` 確定、Step 14: SOUP 正式登録 + C++20 プロジェクト骨格)の進捗に応じて昇格してきた。Step 17+(コード実装)以降はソースコード本体・試験資産の追加に応じて更新する。
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.6 | ドラフト(セルフ承認、Step 14 反映で v0.5 → v0.6 昇格 — SOUP-001/002/003/004/005/006/007/008/009/010/013/014/015 を「予定」→ 正式登録、ツール表 008〜014 / 017、CFG 表 009〜014 を運用中に昇格、§3 SRC-001/005 をプレースホルダ実装段階に) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.12 | 継続更新(Step 0〜14 を時系列記録。v0.7 Step 9、v0.8 Step 10、v0.9 Step 11、v0.10 Step 12、v0.11 Step 13、v0.12 Step 14 に対応) |
