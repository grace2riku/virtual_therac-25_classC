...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 21 完了時点(UNIT-203 BeamController 実装 + UTPR v0.5、CR-0009 / GitHub Issue #9、2026-04-30、RCM-017 中核 / 許可フラグ消費型)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13: SRS + `inc1-requirements-frozen`、Step 14: SOUP 正式登録 + C++20 骨格、Step 15: SAD、Step 16: SDD + `inc1-design-frozen`、Step 17: UNIT-200 実装 + UTPR v0.1、Step 18: UNIT-401 SPSC lock-free queue 実装 + SDD v0.1.1 + UTPR v0.2、Step 19: UNIT-201 SafetyCoreOrchestrator 実装 + UTPR v0.3、Step 20: UNIT-202 TreatmentModeManager 実装 + UTPR v0.4)に応じて昇格してきた。Step 21+(残ユニットの順次実装)以降は SDD §3 の残ユニット(UNIT-203〜210 / 301〜304 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203 BeamController(RCM-017 中核 / 許可フラグ消費型)実装段階に到達**。UNIT-200/401/201/202 に加え、`include/th25_ctrl/beam_controller.hpp` + `src/beam_controller.cpp`(UNIT-203): ビームオン許可フラグの `compare_exchange_strong` による 1 回限り消費型 + BeamState 4 状態機械(Off/Arming/On/Stopping)+ 公開 API 3 種(request_beam_on/request_beam_off/current_state)+ 補助 API 2 種(set_beam_on_permission/is_beam_on_permitted)、`std::atomic<BeamState>` + `std::atomic<bool>` 二重 `is_always_lock_free` `static_assert` 表明。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ完全ビルド成功)) | C | UNIT-204〜210 / 301〜304 / 402 は Step 22+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.5 | ドラフト(セルフ承認、Step 17〜20 で v0.1〜v0.4、**v0.5 昇格 Step 21 / CR-0009** で UNIT-203 BeamController 試験ケース 19 件追加(累積 106 件、RCM-017 中核)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1quinquies / §8.2 試験ケース結果 / §9 結論 / §10 トレーサビリティ表に UNIT-203 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.13 | ドラフト(セルフ承認、Step 21 反映で v0.12 → v0.13 昇格 — CI-SRC-001 を UNIT-200〜UNIT-203 実装段階(RCM-017 中核)に、CI-DOC-UTPR を v0.4 → v0.5(UNIT-203 試験 19 件追加、累積 106 件)に、CI-DOC-CRR を v0.10 → v0.11、CI-DOC-DEVSTEPS を v0.19 → v0.20 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.11 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜21 で CR-0001〜0009 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.20 | 継続更新(Step 0〜21 を時系列記録。v0.7〜v0.19 で Step 9〜20、v0.20 で Step 21(UNIT-203 RCM-017 中核)に対応) |
