...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)SAD-TH25-001 v0.1 完成時点(Step 15、CR-0003 / GitHub Issue #3、2026-04-26)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13: SRS 追加 + `inc1-requirements-frozen` 確定、Step 14: SOUP 正式登録 + C++20 プロジェクト骨格、Step 15: SAD 作成)に応じて昇格してきた。Step 16(SDD)で `inc1-design-frozen` を確定、Step 17+(コード実装)以降はソースコード本体・試験資産の追加に応じて更新する。
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.7 | ドラフト(セルフ承認、Step 15 反映で v0.6 → v0.7 昇格 — CI-DOC-SAD を「未着手」→ v0.1 に昇格、CI-DOC-CRR を v0.4 → v0.5、CI-DOC-DEVSTEPS を v0.12 → v0.14 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.5 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13/14/15 で CR-0001/CR-0002/CR-0003 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.14 | 継続更新(Step 0〜15 を時系列記録。v0.7 Step 9、v0.8 Step 10、v0.9 Step 11、v0.10 Step 12、v0.11 Step 13、v0.12 Step 14、v0.13 Step 14 SHA 追記事後処理、v0.14 Step 15 に対応) |
