...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 29 完了時点(UNIT-304 IonChamberSim 実装 + UTPR v0.13、CR-0017 / GitHub Issue #21、2026-05-04、Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure → **Hardware Simulator 層完成**)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜28)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)が成立、Step 24/25/26/27/28/29 では Step 23 教訓の継続反映により **PRB 起票なし**(Step 29 では Step 26〜28 で確立した Sim 層設計パターン + 多重 atomic 設計の初版採用継続反映によりローカルビルドエラー発生せず、6 連続クリーンコミット成立予定)。Step 30+(Inc.2/Inc.4 該当の空殻ユニット + UNIT-402 InterProcessChannel 実装)以降は SDD §3 の残ユニット(UNIT-207 / 209 / 210 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203(RCM-017 中核)、Step 22 で UNIT-204(HZ-005 直接 + HZ-003 構造的排除の中核)、Step 23 で UNIT-205(HZ-001 直接 + RCM-005 中核 / 3 系統センサ集約 med-of-3)、Step 24 で UNIT-206(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)、Step 25 で UNIT-208 StartupSelfCheck(RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)実装段階に到達**。UNIT-200/401/201/202/203/204/205/206 に加え、`include/th25_ctrl/startup_self_check.hpp` + `src/startup_self_check.cpp`(UNIT-208): SDD §4.9 起動時 4 項目自己診断(電子銃 0 mA / ターンテーブル Light 位置 / 磁石 0 A / ドーズカウンタ 0)を Manager 群 (UNIT-204/205/206) 経由で実装、UNIT-301 ElectronGunSim 未実装のため `inject_electron_gun_current(ElectronGunCurrent_mA)` 補助 API + `std::atomic<double>` 1 個で UT 駆動可能、SRS-012「StartupSelfCheckFailed」を 4 項目それぞれ既存 ErrorCode 階層(Beam/Turntable/Magnet/Internal)で意味的に分類することで `inc1-design-frozen` 維持 + 公開 API 1 種(perform_self_check)+ 補助 API 2 種 + 静的純粋関数 3 種(is_electron_gun_in_zero / is_bending_magnet_in_zero / is_dose_zero)、`static_assert(std::atomic<double>::is_always_lock_free)` を **9 ユニット目に拡大**。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ完全ビルド成功)) | C | UNIT-207 / 209 / 210 / 301〜304 / 402 は Step 26+ で SDD-TH25-001 v0.1.1 に基づき順次追加 |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.13 | ドラフト(セルフ承認、Step 17〜28 で v0.1〜v0.12、**v0.13 昇格 Step 29 / CR-0017** で UNIT-304 IonChamberSim 試験ケース 24 件追加(累積 309 件、Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1terdecies / §8.3 カバレッジ / §8.4 不具合・逸脱 / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-304 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.21 | ドラフト(セルフ承認、Step 29 反映で v0.20 → v0.21 昇格 — **CI-SRC-005 を UNIT-301 + UNIT-302 + UNIT-303 から UNIT-301 + UNIT-302 + UNIT-303 + UNIT-304(Hardware Simulator 層 4 ユニット目 / 本層最終 / 故障注入機能あり)に拡大 → Hardware Simulator 層完成**、CI-DOC-UTPR を v0.12 → v0.13(UNIT-304 試験 24 件追加、累積 309 件)に、CI-DOC-CRR を v0.18 → v0.19、CI-DOC-DEVSTEPS を v0.27 → v0.28 に整合更新) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.17 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜27 で CR-0001〜0015 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.28 | 継続更新(Step 0〜29 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27、v0.27 で Step 28、v0.28 で Step 29(UNIT-304 IonChamberSim / Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure → Hardware Simulator 層完成)に対応) |
