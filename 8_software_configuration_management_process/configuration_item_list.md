...
> **本プロジェクトの位置づけ(注記)**
>
> 本ドキュメントは IEC 62304 に基づく医療機器ソフトウェア開発プロセスの学習・参考実装を目的とした **仮想プロジェクト** の成果物である。本 CIL は **Phase 4(Inc.1)Step 30 完了 + SHA 追記事後処理 + PRB-0005 反映時点(UNIT-207 SafetyMonitor 空殻実装 + UTPR v0.14、CR-0018 / GitHub Issue #22、本体 `84276cc` + 修正 `12008b7`、PRB-0005 / GitHub Issue #24 で UT-208-29 SetterMultiReader を do-while パターン化、2026-05-07、CI run `25446778687` 全 9 ジョブ Pass)** の構成アイテムを網羅する。Phase 2 終了時(v0.1、2026-04-24)で初版、Phase 3 各ステップ(Step 10: CCB/CRR 追加、Step 11: RMF 追加、Step 12: `planning-baseline` 確定)、Phase 4 進捗(Step 13〜29)に応じて昇格してきた。Step 22 では **CI 失敗が初めて発生し SPRP §3.1 PRB 起票プロセスの本格運用第 1 例**(PRB-0001/0002 遡及起票)、Step 23 では **本格運用第 2 例**(PRB-0003/0004 遡及起票)、**Step 30 では本格運用第 3 例**(PRB-0005、UT-208-29 gcc-13/debug 環境固有 flaky を do-while パターン化で構造的予防)が成立。Step 24/25/26/27/28/29 では Step 23 教訓の継続反映により PRB 起票なし(7 連続クリーンコミットは Step 30 で UT-208-29 既存テストの設計上の脆弱性が顕在化したため不成立、ただし空殻実装本体は初版即ビルド成功)。Step 31+(Inc.2/Inc.4 該当の残空殻ユニット UNIT-209 AuditLogger / UNIT-210 CoreAuthenticationGateway + UNIT-402 InterProcessChannel 実装)以降は SDD §3 の残ユニット(UNIT-209 / 210 / 402)を順次追加し、Inc.1 完了時に `inc1-baseline` を確定する。
...
|-------|------|-----------------|-------------|----------|------|
| CI-SRC-001 | ビーム生成・モード制御コア(Inc.1) | `src/th25_ctrl/` | 0.2.0(**Step 17 で UNIT-200、Step 18 で UNIT-401、Step 19 で UNIT-201、Step 20 で UNIT-202(HZ-001 直接対応の中核)、Step 21 で UNIT-203(RCM-017 中核)、Step 22 で UNIT-204(HZ-005 直接 + HZ-003 構造的排除の中核)、Step 23 で UNIT-205(HZ-001 直接 + RCM-005 中核 / 3 系統センサ集約 med-of-3)、Step 24 で UNIT-206(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表)、Step 25 で UNIT-208 StartupSelfCheck(RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断)、Step 30 で UNIT-207 SafetyMonitor 空殻(Inc.2 で本格化予定 / 独立並行タスクのインターロック監視 / 空殻実装としては第 1 例)実装段階に到達**。UNIT-200/401/201/202/203/204/205/206/208 に加え、**`include/th25_ctrl/safety_monitor.hpp` + `src/safety_monitor.cpp`(UNIT-207、Step 30、空殻)**: SDD §4.8 独立並行タスクのインターロック監視ユニットの空殻 IF 骨格として実装、公開 API 2 種(run_monitor_thread / stop)+ 補助 API 3 種(is_stop_requested / tick_count / reset_for_test)+ 定数 1 種(kLoopIntervalMs = 10 ms)、`std::atomic<bool>` `stop_requested_` + `std::atomic<std::uint64_t>` `tick_count_` の **計 2 atomic 変数 / 二重 atomic 種** で並行アクセス保護(空殻範囲の最小構成)、SafetyMonitor 自身は std::thread を所有しない設計(呼出側が別スレッドで run_monitor_thread() を起動)、`static_assert(std::atomic<bool>::is_always_lock_free)` + `static_assert(std::atomic<std::uint64_t>::is_always_lock_free)` の **二重表明** を **14 ユニット目に拡大**(Inc.1 範囲全 14 ユニット展開完了)。**Inc.2 で本格化予定**(MessageBus consume + AuditLogger 転送 + RCM-006 センサ多数決 + RCM-007 2 組ドーズ比較 + 起動時自己診断失敗検知 + SRS-ALM-001/002/003/004/005 の連続 100 ms 以上検出ロジック)。`th25_ctrl` ライブラリの 9 cpp 目として組込。ローカル骨格ビルドで全ヘッダの `static_assert` 全件通過(AppleClang 15、`th25_ctrl` 静的ライブラリ完全ビルド成功)) | C | UNIT-209 / 210 / 402 は Step 31+ で SDD-TH25-001 v0.1.1 に基づき順次追加(空殻実装としては UNIT-207 が第 1 例、UNIT-209/210 も同パターン継続予定)、UNIT-301〜304 は `src/th25_sim/` 配下で別 CI(CI-SRC-005、Hardware Simulator 層完成済) |
...
| CI-DOC-UTPR | ユニットテスト計画書/報告書 | `5.5_software_unit_implementation/software_unit_test_plan_report.md` | 0.14 | ドラフト(セルフ承認、Step 17〜29 で v0.1〜v0.13、**v0.14 昇格 Step 30 / CR-0018** で UNIT-207 SafetyMonitor 空殻試験ケース 12 件追加(累積 321 件、Inc.2 で本格化予定の独立並行タスクのインターロック監視ユニット 空殻実装、空殻実装としては第 1 例)。§3.2 実装対象表 / §7.2 試験ケース定義 / §8 第 II 部実施サマリ §8.1quaterdecies / §8.3 カバレッジ / §8.4 不具合・逸脱 / §8.5 未達項目 / §9 結論 / §10 トレーサビリティ表に UNIT-207 セクション追加) |
...
| CI-DOC-CIL | 構成アイテム一覧(本書) | `8_software_configuration_management_process/configuration_item_list.md` | 0.23 | ドラフト(セルフ承認、Step 30 SHA 追記事後処理 + PRB-0005 反映で v0.22 → v0.23 昇格 — CI-DOC-CRR を v0.20 → v0.21(CR-0018 状態 CLOSED + PRB-0005 関連 PRB 列追記 + コミット `84276cc` / `12008b7` + CI run id `25446778687` / `25446778695` 記録)、CI-DOC-DEVSTEPS を v0.29 → v0.30 に整合更新、SPRP §3.1 PRB 起票プロセス本格運用第 3 例) |
...
| CI-DOC-CRR | 変更要求台帳 | `8_software_configuration_management_process/change_request_register.md` | 0.21 | ドラフト(セルフ承認、Step 10 空台帳 → Step 13〜30 で CR-0001〜0018 を §4 本体表に登録、Therac-25 事故主要因類型 A〜F 別集計表新設、CR-0018 SHA 追記事後処理 + PRB-0005 反映完了、SPRP §3.1 PRB 起票プロセス本格運用第 3 例) |
...
| CI-DOC-DEVSTEPS | 開発ステップ記録(お手本) | `DEVELOPMENT_STEPS.md` | 0.30 | 継続更新(Step 0〜30 を時系列記録。v0.7〜v0.24 で Step 9〜25、v0.25 で Step 26、v0.26 で Step 27、v0.27 で Step 28、v0.28 で Step 29(UNIT-304 IonChamberSim / Hardware Simulator 層完成)、v0.29 で Step 30(UNIT-207 SafetyMonitor 空殻 / Inc.2 で本格化予定の独立並行タスクのインターロック監視 / 空殻実装としては第 1 例)、v0.30 で Step 30 SHA 追記事後処理 + PRB-0005 反映(SPRP §3.1 PRB 起票プロセス本格運用第 3 例)に対応) |
