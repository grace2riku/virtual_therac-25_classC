...
>
> Phase 0〜2(プロジェクト初期化〜支援プロセス計画)で実施した活動(Step 0〜9: テンプレート複製、SSC/SDP/README/CLAUDE/DEVELOPMENT_STEPS/UPSTREAM_FEEDBACK/SRMP/SCMP/SPRP/SMP/CIL の作成)は **本規程施行前の「Phase 1〜2 プロジェクト立ち上げ活動」** として扱い、本台帳への遡及登録は行わない(代わりに `DEVELOPMENT_STEPS.md` にステップ記録として残る)。本規程施行後の最初の CR が `CR-0001` となる(Phase 3 で RMF 初版作成時の関連 CR、または Inc.1 着手時の最初の機能追加 CR が候補)。
...
| [6] | リスクマネジメントファイル(RMF-TH25-001) ※作成予定 | — |
| [7] | 構成アイテム一覧(CIL-TH25-001) | 0.1 |
...
> **CCB-TH25-001 v0.1 施行(2026-04-24)後の最初の CR がここに記録される。** 本台帳は空状態で運用開始する。Phase 0〜2 の活動(Step 0〜9)は本規程施行前の立ち上げ活動として遡及登録対象外(`DEVELOPMENT_STEPS.md` にて記録済)。
...
|-------|------|------|------------------|-------------|----------------|----------------|
| _(エントリ無し — Phase 0〜2 までの活動は本規程施行前につき非対象)_ | | | | | | |
