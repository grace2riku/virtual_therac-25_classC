...
| CR-0008 | 2026-04-30 | k-abe | MAJOR | VERIFICATION | Inc.1 Step 20 — UNIT-202 TreatmentModeManager 実装(モード遷移許可表 + 強い型エネルギー範囲検証 + BeamState 事前条件、HZ-001 直接対応の中核)+ UTPR v0.4(IEC 62304 §5.5.x クラス C 必須)| — | CI-SRC-001(UNIT-200+UNIT-401+UNIT-201+UNIT-202 実装段階)/ CI-DOC-UTPR(v0.4 昇格)/ CI-DOC-CIL / CI-DOC-CRR / CI-DOC-DEVSTEPS / CI-DOC-README | RCM-001(モード遷移整合性、HZ-001 中核)/ RCM-005(in-position 冗長確認、構造的前提)/ RCM-008(強い型による単位安全)| **HZ-001(直接、Therac-25 Tyler 事故型の中核対策)** / HZ-002(構造的予防、`std::atomic<TreatmentMode>` + release-acquire ordering)/ HZ-005(部分対応、強い型 `Energy_MeV` / `Energy_MV`)/ HZ-007(構造的予防、`is_always_lock_free` `static_assert`)| A(構造的予防、`std::atomic<TreatmentMode>` + UT-202-21 で 2 writer + 1 reader × 1000 イテレーション race-free 検証 TSan)/ **D(構造的前提、Therac-25 Tyler 事故型「Electron モードで X 線エネルギー」「BeamState 確認漏れ」の発現経路を UT-202-09〜11 / UT-202-18〜19 / UT-202-25 88 組合せ網羅で機械検証成立)** / F(構造的予防、ヘッダ内 `static_assert(std::atomic<TreatmentMode>::is_always_lock_free)` で HZ-007 機械的予防を 4 ユニット目に拡大)| 直接コミット: (Step 20 push 後追記、SCMP §4.1.2 admin bypass 例外条件下)| UTPR v0.4 §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目 全クリア + §7.2 UNIT-202 試験ケース 26 件(うち UT-202-25 は SDD §4.3 表との 9 組合せ網羅機械検証、UT-202-21 は 2 writer + 1 reader race-free 試験、UT-202-18〜19 は Therac-25 Tyler 事故型の API ガード機械検証)**CI cpp-build マトリクス全 8 ジョブ Pass + clang-tidy 警告 0 は push 後の SHA 追記事後処理で確認** | TSan(`clang-17/tsan` + `gcc-13/tsan`)/ ASan(`*/asan`)/ UBSan(`*/ubsan`)期待値 全 6 ジョブ Pass、UT-202-21 race detection 0(`std::atomic<TreatmentMode>` の release-acquire ordering 機械検証)| Yes(1 分以上、実装 + UT + UTPR 昇格所要時間で自然に充足、CCB §5.4。Issue 起票 2026-04-30T12:30:02Z)| 不要(本 CR は SDD §4.3 計画済の RCM-001/005/008 を実装するのみ、HZ/RCM 自体の追加・変更なし)| (push + CI Pass 確認後に追記)| GitHub Issue #8。CCB-TH25-001 v0.1 本格運用第 8 例。`inc1-design-frozen`(BL-20260426-002)維持(SDD §4.3 と完全一致、構造的要件は不変)。**HZ-001 直接対応の中核ユニット**: Therac-25 Tyler 事故型の三重エラー(Electron モードで X 線エネルギー / BeamState 確認漏れ / ターンテーブル位置不整合)の発現経路を構造的に拒否|
...
| Step 20 完了時 | 0(本 Step push 後の CI 結果で確定。期待値: `clang-17/tsan` + `gcc-13/tsan` で UT-202-21(2 writer + 1 reader × 1000 イテレーション)race-free 検証成立、Inc.1 Manager 層への HZ-002 機械的予防拡大、UT-202-25 で SDD §4.3 表との 9 組合せ網羅機械検証成立、UT-202-18〜19 で Therac-25 Tyler 事故型 API ガード機械検証成立)| 0 | 0 | 0 | 100%(期待値、push 後 SHA 追記事後処理で確定。累積 87 件 UT)|
| Inc.1 完了時 | _(未実施)_ | | | | |
