...
| CR-0006 | 2026-04-29 | k-abe | MAJOR | VERIFICATION | Inc.1 Step 18 — UNIT-401 InProcessQueue (SPSC lock-free) 実装 + SDD §4.13 軽微修正(folly→独自実装)+ UTPR v0.2(IEC 62304 §5.5.x クラス C 必須)| — | CI-SRC-001(UNIT-200+UNIT-401 実装段階)/ CI-DOC-SDD(v0.1.1 軽微改訂)/ CI-DOC-UTPR(v0.2 昇格)/ CI-DOC-CIL / CI-DOC-CRR / CI-DOC-DEVSTEPS / CI-DOC-README | RCM-002(release-acquire メモリオーダリング)/ RCM-004(`std::atomic` 共有変数のみ)/ RCM-019(SPSC lock-free queue で SDD §9 SEP-003 実装)| HZ-002(構造的予防、SPSC release-acquire + concurrent UT) / HZ-007(構造的予防、`is_always_lock_free` + power-of-two `static_assert`)| A(構造的予防、SPSC ring buffer 実装 + UT-401-12 で 1 producer + 1 consumer × 100k メッセージ race-free 検証 TSan 必須) / F(構造的予防、ヘッダ内 `static_assert(is_always_lock_free)` で HZ-007 機械的予防成立、power-of-two 制約も `static_assert` 化)| 直接コミット(SCMP §4.1.2 admin bypass 例外条件下、Step 18 本体コミット時に SHA 追記) | UTPR v0.2 §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目 全クリア(計画段階)+ §7.2 UNIT-401 試験ケース 13 件設計済 + ローカル骨格ビルドで `static_assert(is_always_lock_free)` + power-of-two 制約 + default-constructible 制約 検証成功(AppleClang 15)| TSan(`clang-17/tsan` + `gcc-13/tsan`)/ ASan / UBSan は CI cpp-build マトリクス全 8 ジョブで実行(本 CR では UT-401-12 が `tsan` プリセットで race-free を検証する設計、実検出 0 を期待)| Yes(1 分以上、実装 + UT + UTPR 昇格 + SDD 軽微修正所要時間で自然に充足、CCB §5.4。Issue 起票 2026-04-29T21:21:44Z)| 不要(本 CR は SDD §4.13 計画済の RCM-002/004/019 を実装するのみ、HZ/RCM 自体の追加・変更なし。SDD §4.13 の実装手段選択は §10 未確定事項として明示されていたものを確定)| (Step 18 完了時)| GitHub Issue #6。CCB-TH25-001 v0.1 本格運用第 6 例。`inc1-design-frozen`(BL-20260426-002)維持(SDD §9 SEP-003 構造的要件は不変、SDD §4.13 実装手段の選択を `folly::ProducerConsumerQueue` → 独自 SPSC 実装(Lamport 1983)に確定するのみ。SDD v0.1 → v0.1.1 軽微改訂)|
...
| Step 18 完了時 | **0 期待(UNIT-401 UT-401-12 が tsan プリセットで 1 producer + 1 consumer × 100k メッセージ race-free 実行する設計、CI cpp-build push 後に確定)**| 0 期待 | 0 期待 | 0 期待 | **100% 期待(全 8 build ジョブ Pass + clang-tidy 警告 0、累積 37 件 UT、SHA 追記事後処理時に確定)**|
| Inc.1 完了時 | _(未実施)_ | | | | |
