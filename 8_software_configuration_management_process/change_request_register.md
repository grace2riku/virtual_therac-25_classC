...
| CR-0007 | 2026-04-30 | k-abe | MAJOR | VERIFICATION | Inc.1 Step 19 — UNIT-201 SafetyCoreOrchestrator 実装(LifecycleState 8 状態機械 + イベントループ + 公開 API 4 種)+ UTPR v0.3(IEC 62304 §5.5.x クラス C 必須)| — | CI-SRC-001(UNIT-200+UNIT-401+UNIT-201 実装段階)/ CI-DOC-UTPR(v0.3 昇格)/ CI-DOC-CIL / CI-DOC-CRR / CI-DOC-DEVSTEPS / CI-DOC-README | RCM-001(状態機械による不正遷移の構造的排除)/ RCM-013(起動時自己診断)/ RCM-017(緊急停止)/ RCM-019(InProcessQueue 経由イベント駆動)| HZ-001(直接、状態機械の RCM-001 中核)/ HZ-002(構造的予防、`std::atomic<LifecycleState>` + InProcessQueue)/ HZ-007(構造的予防、`is_always_lock_free` `static_assert`)| A(構造的予防、SPSC イベント配送 + `std::atomic<LifecycleState>` release-acquire + UT-201-24 で 200 サイクル × 6 イベント race-free 検証 TSan)/ D(構造的前提、不正遷移は Halted 強制 + 物理的再起動要求、UT-201-18/19/23 で機械検証)/ F(構造的予防、ヘッダ内 `static_assert(std::atomic<LifecycleState>::is_always_lock_free)` で HZ-007 機械的予防成立)| 直接コミット: (Step 19 push 後追記、SCMP §4.1.2 admin bypass 例外条件下)| UTPR v0.3 §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目 全クリア + §7.2 UNIT-201 試験ケース 24 件(うち UT-201-21 は SDD §6.1 表との 88 組合せ網羅機械検証、UT-201-24 は SPSC イベント配送 race-free 試験)。**CI cpp-build マトリクス全 8 ジョブ Pass + clang-tidy 警告 0 は push 後の SHA 追記事後処理で確認** | TSan(`clang-17/tsan` + `gcc-13/tsan`)/ ASan(`*/asan`)/ UBSan(`*/ubsan`)期待値 全 6 ジョブ Pass、UT-201-24 race detection 0(SDD §9 SEP-003 + RCM-019 のオーケストレータ層拡大の機械検証)| Yes(1 分以上、実装 + UT + UTPR 昇格所要時間で自然に充足、CCB §5.4。Issue 起票 2026-04-29T22:41:50Z)| 不要(本 CR は SDD §4.2 / §6.1 計画済の RCM-001/013/017/019 を実装するのみ、HZ/RCM 自体の追加・変更なし)| (push + CI Pass 確認後に追記)| GitHub Issue #7。CCB-TH25-001 v0.1 本格運用第 7 例。`inc1-design-frozen`(BL-20260426-002)維持(SDD §4.2 / §6.1 と完全一致、構造的要件は不変)|
...
| Step 19 完了時 | 0(本 Step push 後の CI 結果で確定。期待値: `clang-17/tsan` + `gcc-13/tsan` で UT-201-24(SPSC イベント配送 200 サイクル × 6 イベント / サイクル)race-free 検証成立、Inc.1 オーケストレータ層への HZ-002 機械的予防拡大)| 0 | 0 | 0 | 100%(期待値、push 後 SHA 追記事後処理で確定。累積 61 件 UT)|
| Inc.1 完了時 | _(未実施)_ | | | | |
