...
| v0.1.1 軽微改訂(セルフレビュー、Step 18 / CR-0006) | 承認(§4.13 / §6.6 / §7 / §10 で folly 採用 → 独自 SPSC 実装(Lamport ring buffer)に確定。SDD 構造的要件 = SPSC + lock-free + `is_lock_free()` 表明 + メッセージパッシング = は不変、`inc1-design-frozen` 維持。実装手段の選択を SDD §10 未確定事項から確定へ) | 2026-04-29 | 本書 v0.1.1 のコミット `2777378` |
