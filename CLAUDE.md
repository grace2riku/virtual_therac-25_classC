...
### SHA 追記事後処理チェックリスト(必須)

本節は SHA 追記事後処理(Step 40 パターン、本体コミット push 後に CI 実績を確定値として各文書に追記する作業)で **必ず更新すべきテーブル** を列挙する。本ルールは PRB-0009 / CR-0032(Step 46、2026-05-15)で制定された。

**背景:** Step 42〜45 の SHA 追記事後処理時に、CRR §5 安全関連 CR 抽出 / §7・§7.1・§7.2 集計表 / DEVELOPMENT_STEPS.md 改訂履歴 / UTPR §7.2 累積合計・§10 トレーサビリティの更新が systematic に漏れた。各文書の改訂履歴エントリには「§7 集計表に行追加」等と記載されていたにもかかわらず実テーブルに反映されておらず、**改訂履歴記載と実体の乖離**が発生していた(PRB-0009、新類型「SHA 追記事後処理時の集計系ドキュメントテーブル更新漏れ」)。CI(`docs-check.yml`)は markdownlint / リンク / 日付書式を検証するが **ドキュメント間の集計値整合性は検証しない** ため、CI でも検出されなかった。

**運用ルール:**

1. **SHA 追記事後処理(本体 push 後)では、以下のテーブルを必ず同一コミットで更新する:**
   - **CRR §4 CR 本体表**: 当該 CR 行の状態 OPEN → CLOSED + 直接コミット欄(本体 SHA)+ CI run id + クローズ日
   - **CRR §5 安全関連 CR 抽出**: 当該 CR 行(新規追加)
   - **CRR §6 リリース別 CR サマリ**: 当該 CR 行(新規追加)
   - **CRR §7 集計表 / §7.1 主要因類型別 / §7.2 Sanitizer 関連**: 当該 Step 完了時行(新規追加または実績値更新)
   - **UTPR §7.2 累積合計試験ケース**: UT 件数を加算した正しい累積値
   - **UTPR §10 トレーサビリティマトリクス**: 新規 UT 行
   - **各文書の改訂履歴テーブル**: 当該バージョンのエントリ
   - **DEVELOPMENT_STEPS.md 改訂履歴テーブル**: 当該バージョンのエントリ(セクション本文の追記だけで満足しない)
2. **「改訂履歴に『更新した』『行追加』と書いたら、実テーブルも必ず同一コミットで更新する」**:改訂履歴エントリの記載と実テーブルの内容が乖離してはならない。記載予定のテーブル更新は、その改訂で完了させる(「後の事後処理で更新予定」と書いた場合は、その事後処理で必ず実テーブルに反映する)。
3. **独立 Step(SHA 追記事後処理のみの Step、Step 38 / Step 42 等)を経由する場合も、集計系テーブルの更新を漏らさない**:独立 Step は本体表のクローズ作業に集中しがちだが、§5/§7 系の更新を同時に行う。
4. **SHA 追記事後処理が完了したら、当該 Step の更新対象テーブルを上から順に確認し、網羅性を機械的にセルフチェックする。**

**集計系テーブルの正本性:** CRR §7・§7.1・§7.2 は Inc. 完了時の傾向分析(SPRP §8 連携)の基礎データであり、§5 は RMF-TH25-001 との整合確認の対象である。これらの集計値が実体と乖離すると、お手本ドキュメントとしての CRR の信頼性を損なう。本チェックリストは CI で検出できない「集計値整合性」を運用ルールで補完する位置づけとする。
