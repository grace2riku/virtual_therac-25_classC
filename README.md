...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.8(Phase 4(Inc.1)`inc1-design-frozen` 確定時点。`inc1-requirements-frozen`(BL-20260426-001)+ `inc1-design-frozen`(BL-20260426-002)確定済。SOUP 13 件正式登録(vcpkg ベースライン commit `c3867e714d...` で完全固定、TSan ランタイムを HZ-002 直接対応として CI 常時実行)、ツール 8 件運用中、構成定義 6 件運用中、ドキュメント表で SAD v0.1 + SDD v0.1 を反映、§10.1 planning-baseline / §10.2 inc1-requirements-frozen / §10.3 inc1-design-frozen 詳細スナップショット) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.6(Step 13/14/15/16 で CR-0001(Inc.1 SRS、CLOSED)/ CR-0002(Inc.1 C++20 骨格、CLOSED)/ CR-0003(Inc.1 SAD、CLOSED)/ CR-0004(Inc.1 SDD + `inc1-design-frozen`、VERIFICATION)を §4 本体表に登録。4 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:4 / **B:1 新規(構造的排除)** / D:2 / E:2 / F:4 いずれも構造的予防または構造的排除/前提。集計表に Step 13/14/15/16 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.8)§5/§6/§7 を参照。
