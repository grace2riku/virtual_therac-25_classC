...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.7(Step 17 で初版、Step 18〜22 で v0.2〜v0.6、Step 23 / CR-0011 で v0.7 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 163 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25(HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` で実装済**。後続 Step 24+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.15(Phase 4(Inc.1)Step 23 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-205 実装段階(**HZ-001 直接対応の中核 + RCM-017 中核 + HZ-005 直接 + HZ-003 構造的排除の中核 + RCM-005 中核 / 3 系統センサ集約**)に昇格、CI-DOC-UTPR を v0.6 → v0.7 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.13(Step 13〜23 で CR-0001〜0011 を §4 本体表に登録。11 件すべて MAJOR・安全関連、CR-0001〜0010 CLOSED + CR-0011 VERIFICATION(UNIT-205 + UTPR v0.7、**HZ-001 直接対応 + RCM-005 中核 / 3 系統センサ集約 med-of-3**)。Therac-25 主要因類型: 累積 A:11 / B:3 / **D:6 新規** / E:2 / F:11。集計表に Step 13〜23 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)が成立) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.15)§5/§6/§7 を参照。
