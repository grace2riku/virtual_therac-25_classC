...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.17(Step 17 で初版、Step 18〜31 で v0.2〜v0.15、Step 32 / CR-0020 で v0.16 昇格、Step 32 SHA 追記事後処理 + PRB-0006 反映で v0.17 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 345 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12(Inc.4 で本格化予定の操作者認証ゲートウェイユニット、空殻実装としては第 3 例))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` + `test_electron_gun_sim.cpp` + `test_bending_magnet_sim.cpp` + `test_turntable_sim.cpp`(本 v0.17 で UT-303-24 を do-while パターンに修正、PRB-0006)+ `test_ion_chamber_sim.cpp` + `test_safety_monitor.cpp` + `test_audit_logger.cpp` + `test_core_authentication_gateway.cpp` で実装済**。後続 Step 33+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.27(Phase 4(Inc.1)Step 32 完了 + SHA 追記事後処理 + PRB-0006 反映時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-001 を UNIT-200/401/201/202/203/204/205/206/208/207/209 から + UNIT-210 CoreAuthenticationGateway 空殻(Inc.4 で本格化予定 / 操作者認証ゲートウェイ / 空殻実装としては第 3 例)に拡大、`th25_ctrl` ライブラリの 11 cpp 目として組込**、CI-SRC-005 は UNIT-301〜304 で Hardware Simulator 層完成済、CI-DOC-UTPR v0.17、CI-DOC-CRR を v0.24 → v0.25(CR-0020 CLOSED + 本体 `b460dde` + 修正 `6c4b176` + CI run `25558831075` 全 9 ジョブ Pass)、CI-DOC-DEVSTEPS を v0.33 → v0.34 に昇格、本体 push 後の CI で UT-303-24 flaky → **PRB-0006 起票・修正 `6c4b176` で解決**(SPRP §3.1 PRB 起票プロセス本格運用第 4 例、Step 30 PRB-0005 と完全同一の根本原因)) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.25(Step 13〜32 で CR-0001〜0020 を §4 本体表に登録 + Step 32 SHA 追記事後処理完了。**CR-0001〜0020 全 CLOSED**(CR-0020 は本体 `b460dde` + PRB-0006 修正 `6c4b176` で全 9 ジョブ Pass、CI run `25558831075`)。20 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:20 / B:3 / D:9 / **E:4**(UNIT-210 で「未認証時拒否」インタフェース契約を IF 骨格段階で確立、Inc.4 で本格化)/ **F:20**(UNIT-210 で `static_assert` 単一表明を **16 ユニット目に拡大、Inc.1 範囲全 14 ユニット + 空殻ユニット第 3 例**)。集計表に Step 13〜32 完了時行を追加。**Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)、Step 30 で第 3 例(PRB-0005)、Step 32 で第 4 例(PRB-0006、UT-303-24 clang-17/debug 環境固有 flaky、Step 30 PRB-0005 と完全同一の根本原因)が成立**。Step 24〜29 / Step 31 では教訓の継続反映により PRB 起票なし、**Step 30 / Step 32 で PRB-0005 / PRB-0006 が同パターンで連続発現したことから教訓水平展開を別 CR で改善検討予定**) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.27)§5/§6/§7 を参照。
