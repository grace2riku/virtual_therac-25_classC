...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.37(Step 17 で初版、Step 18〜44 事後処理で v0.2〜v0.35、**Step 45 / CR-0031 で v0.36、Step 45 SHA 追記事後処理で v0.37 昇格**(UNIT-201 init_subsystems() → UNIT-208 perform_self_check() 結線、§3.2 UNIT-201 行に Step 45 範囲拡張注記 + §7.2 UT-201-29〜32 4 件追加 累積 UNIT-201 32 件 + §10 トレーサビリティ行を CI 実績 run id `25866270244` で確定 + §11 改訂履歴 v0.36/v0.37)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 418 件を定義**(v0.36 で 405 件据え置きの累積合計を正値 418 件に修正)、後続 Step Inc.1 後半 / Step Inc.1 完了で Manager/Sim 結線 + IT-101 < 10 ms 実時間実測 + ITPR 作成を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.48(Phase 4(Inc.1)Step 45 SHA 追記事後処理時点(CR-0031 / GitHub Issue #40、UNIT-201 init_subsystems() → UNIT-208 perform_self_check() 結線、CR-0031 状態 OPEN → CLOSED 遷移完了、本体 `0e52c49` + CI run id `25866270244` C++ Build (Inc.1) 全 9 ジョブ Pass ✅ + `25866270321` Documentation Checks success ✅、**1 コミット初回 CI Pass 復活第 4 例成立**)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-201 init_subsystems() → UNIT-208 結線に拡張、CI-DOC-UTPR を v0.36 → v0.37、CI-DOC-CRR を v0.45 → v0.46、CI-DOC-DEVSTEPS を v0.54 → v0.55。派生ドキュメント整合化中に DEVSTEPS 改訂履歴 v0.50〜v0.53 欠落 + CRR §5/§7/§7.1/§7.2 集計系テーブル更新漏れを発見、DEVSTEPS 改訂履歴はバックフィル済 + CRR 集計系は PRB-0009 起票候補として記録) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.46(Step 13〜45 で CR-0001〜0031 を §4 本体表に登録。**CR-0001〜0031 全 CLOSED**(Step 45 SHA 追記事後処理で CR-0031 状態 OPEN → CLOSED 遷移完了、本体 `0e52c49` + CI run id `25866270244`(C++ Build (Inc.1) 全 9 ジョブ Pass ✅、2026-05-14)+ `25866270321`(Documentation Checks success ✅)+ Sanitizer 全 Pass + クローズ日 2026-05-14)。31 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:28 / B:3 / D:10 / **E:9** / **F:29**(CR-0031 は D 構造的予防。なお §5/§7/§7.1/§7.2 集計表の Step 42〜45 分反映は PRB-0009 起票候補として Step 46+ で一括バックフィル予定)。**PRB-0007 + PRB-0008 起票 2 件 + PRB-0009 起票候補 1 件**(SPRP §3.1 本格運用、PRB-0009 候補は新類型「SHA 追記事後処理時の集計系ドキュメントテーブル更新漏れ」、DEVSTEPS 改訂履歴 + CRR §5/§7/§7.1/§7.2 集計系テーブルの更新漏れを Step 45 で発見、Step 46+ で正式起票 + 教訓水平展開予定)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.48)§5/§6/§7 を参照。
