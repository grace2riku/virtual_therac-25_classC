...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.9(Step 17 で初版、Step 18〜24 で v0.2〜v0.8、Step 25 / CR-0013 で v0.9 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 223 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29(RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` で実装済**。後続 Step 26+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.17(Phase 4(Inc.1)Step 25 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-208 実装段階(**HZ-001 直接対応の中核 + RCM-017 中核 + HZ-005 直接 + HZ-003 構造的排除の中核 + RCM-005 中核 / 3 系統センサ集約 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表 + RCM-013 中核 / 起動時 4 項目自己診断**)に昇格、CI-DOC-UTPR を v0.8 → v0.9 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.15(Step 13〜25 で CR-0001〜0013 を §4 本体表に登録。13 件すべて MAJOR・安全関連、CR-0001〜0012 CLOSED + CR-0013 VERIFICATION(UNIT-208 + UTPR v0.9、**RCM-013 中核 + HZ-009 部分対応 / 起動時 4 項目自己診断**)。Therac-25 主要因類型: 累積 A:13 / B:3 / **D:7 新規** / E:2 / **F:13 新規**(UNIT-208 で `static_assert` を 9 ユニット目に拡大)。集計表に Step 13〜25 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)が成立、Step 24/25 では Step 23 教訓の継続反映により PRB 起票なし) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.17)§5/§6/§7 を参照。
