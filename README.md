...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.27(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、**Step 39 / CR-0026(教訓水平展開 / 改善 CR、PRB-0007 連動)で v0.27 昇格**(§8.4 に教訓水平展開記録追加、全 UT-XXX-08 9 ユニットを SDD §6.2 と一斉照合し不整合なし確認、§11 改訂履歴 v0.27)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 393 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12 + UNIT-103 12 + UNIT-104 12)**。本体 `918df6c` + 修正 `37aa742`(UT-104-08 Severity 期待値 Critical → Medium 訂正)で CI run `25626279459` 全 9 ジョブ Pass + `25626279447` success 確定。後続 Step 40+ で UNIT-402 / Inc.1 後半 dispatch 機構等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.38(Phase 4(Inc.1)Step 39 完了時点(教訓水平展開 / 改善 CR、CR-0026、PRB-0007 連動)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 / CI-SRC-004(UNIT-101〜104 空殻、`th25_ui` 4 cpp 目)/ CI-SRC-005 は本 Step 39 では変更なし(運用ルール追加 + ドキュメント整合化のみ)、CI-DOC-UTPR を v0.26 → v0.27、CI-DOC-CRR を v0.35 → v0.36(CR-0026 登録、本格運用第 26 例、CR-0001〜0026 全 CLOSED)、CI-DOC-DEVSTEPS を v0.44 → v0.45、CI-DOC-SPRP を v0.1 → v0.2、CI-DOC-CLAUDE に「UT 作成時の Severity マッピング自己セルフチェック」運用ルール追加を反映。**SPRP §3.1 PRB 起票プロセスの教訓水平展開運用ルール本格運用第 2 例**(CR-0021 do-while パターン化に続く)) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.36(Step 13〜39 で CR-0001〜0026 を §4 本体表に登録。**CR-0001〜0026 全 CLOSED**(CR-0026 は **教訓水平展開 / 改善 CR**、本番コード / UT コード変更なし、運用ルール追加 + ドキュメント整合化のみ、CI は Documentation Checks のみ実行見込み)。26 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:25 / B:3 / D:9 / **E:8** / **F:25**(CR-0026 で F 系 +1)。集計表に Step 13〜39 完了時行を追加。**PRB-0007 起票 1 件**(SPRP §3.1 本格運用第 5 例、教訓水平展開運用ルール本格運用第 2 例、CR-0021 に続く)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.38)§5/§6/§7 を参照。
