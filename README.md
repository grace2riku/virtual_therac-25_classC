...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.30(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28(UNIT-402 セクション + 12 件 UT 追加)、Step 40 SHA 追記事後処理で v0.29、**Step 41 / CR-0028 で v0.30 昇格**(SafetyEventObserver(共通基盤)行を §3.2 に追加 + UNIT-204 行に Step 41 範囲拡張の注記追加 + UT-204-33〜37 5 件追加(累積 410 件)、§7.2 / §8.1duovicies / §10 に UNIT-204 拡張記録追加、§11 改訂履歴 v0.30)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 410 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 37 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12 + UNIT-103 12 + UNIT-104 12 + UNIT-402 12)**、`safety_event_observer.hpp`(ヘッダオンリー abstract interface 新規追加)+ UT-204-33〜37 を `test_dose_manager.cpp` に追加。後続 Step 42+ で UNIT-201 SafetyCoreOrchestrator 結線 + UNIT-203 BeamController dispatch を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.41(Phase 4(Inc.1)Step 41 完了時点(CR-0028 SafetyEventObserver 基盤導入、Observer pattern 基盤のみ、UNIT-204 DoseManager に observer 通知点追加、UNIT-201 結線・UNIT-203 結線は Step 42+ に分離、新規 ヘッダオンリー abstract interface `safety_event_observer.hpp` 追加 + UNIT-204 DoseManager 拡張 + UT-204-33〜37 5 件追加(累積 410 件))。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-001 を `safety_event_observer.hpp` + UNIT-204 DoseManager 拡張に拡大**、CI-DOC-UTPR を v0.29 → v0.30、CI-DOC-CRR を v0.38 → v0.39(CR-0028 を §4 本体表に登録、本格運用第 28 例、push 後 SHA 追記事後処理で CLOSED 化予定)、CI-DOC-DEVSTEPS を v0.47 → v0.48) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.39(Step 13〜41 で CR-0001〜0028 を §4 本体表に登録。**CR-0001〜0027 全 CLOSED + CR-0028 OPEN**(CR-0028 は Step 41 SafetyEventObserver 基盤導入、本格運用第 28 例、push 後 SHA 追記事後処理で CLOSED 化予定)。28 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:27 / B:3 / D:9 / **E:9** / **F:27**(CR-0028 で A/F 系構造的予防に +1)。集計表に Step 13〜41 完了時行を追加。**PRB-0007 起票 1 件**(SPRP §3.1 本格運用第 5 例)) |
