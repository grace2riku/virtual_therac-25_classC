...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.29(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28(UNIT-402 セクション + 12 件 UT 追加、累積 405 件)、**Step 40 SHA 追記事後処理で v0.29 昇格**(§8.1unvicies コミット欄 `a266ec1` 確定 + CI 結果「全 9 ジョブ success ✅」確定 + UT-402-11 race detection 0 確定 + UT-402-07/08/12 機械検証成立確定 + 1 コミット初回 CI Pass 復活第 1 例成立記録、§11 改訂履歴 v0.29)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 405 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12 + UNIT-103 12 + UNIT-104 12 + UNIT-402 12)**、`test_inter_process_channel.cpp` 新規追加。後続 Step 41+ で Inc.1 後半 SafetyCoreOrchestrator dispatch 機構整備等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.40(Phase 4(Inc.1)Step 40 完了 + SHA 追記事後処理時点(CR-0027 UNIT-402 InterProcessChannel 空殻実装、Inc.4 で本格化、空殻実装第 8 例、本体 `a266ec1`、CI run `25770157472` 全 9 ジョブ Pass + Documentation Checks `25770157433` success、Step 33-36 連続 1 コミット初回 CI Pass ストリーク(Step 37 で途絶)からの復活第 1 例成立)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-001 を UNIT-200〜UNIT-210 + UNIT-402 InterProcessChannel 空殻に拡大**(`th25_ctrl` ライブラリの 12 cpp 目組込、空殻実装第 8 例、HZ-006 中核 + RCM-018 中核 IPC ラッパ、CLAUDE.md / CR-0026 制定の Severity マッピング自己セルフチェック運用ルール初適用例)、CI-DOC-UTPR を v0.28 → v0.29、CI-DOC-CRR を v0.37 → v0.38(CR-0027 CLOSED + 本体 SHA + CI run id + Sanitizer 全 Pass 確定)、CI-DOC-DEVSTEPS を v0.46 → v0.47) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.38(Step 13〜40 で CR-0001〜0027 を §4 本体表に登録。**CR-0001〜0027 全 CLOSED**(CR-0027 は Step 40 UNIT-402 InterProcessChannel 空殻実装、本格運用第 27 例、本 SHA 追記事後処理で OPEN → CLOSED 遷移完了、本体 `a266ec1` + CI run id `25770157472` / `25770157433` 確定 + Sanitizer 全 Pass 確定 + クローズ日 2026-05-13)。27 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:26 / B:3 / D:9 / **E:9** / **F:26**。集計表に Step 13〜40 完了時行を追加(Step 40 完了時行は実績値に更新、1 コミット初回 CI Pass 復活第 1 例成立を記録)。**PRB-0007 起票 1 件**(SPRP §3.1 本格運用第 5 例)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.39)§5/§6/§7 を参照。
