...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.24(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 381 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12 + UNIT-103 12(Inc.4 で本格化予定のアラーム/エラー表示 UI ユニット、空殻実装としては第 6 例、既存ライブラリ `th25_ui` の 3 cpp 目組込、RCM-009 中核))**、`test_alarm_display.cpp` 新規追加。後続 Step 37+ で UNIT-104 / UNIT-402 等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.35(Phase 4(Inc.1)Step 36 完了 + SHA 追記事後処理時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-004 を Step 36 で UNIT-101 + UNIT-102 + UNIT-103 AlarmDisplay 空殻に拡大**(`th25_ui` ライブラリの 3 cpp 目組込、新ライブラリ追加なし、空殻実装としては第 6 例、RCM-009 中核)、CI-SRC-001 / CI-SRC-005 は変更なし、CI-DOC-UTPR v0.24、CI-DOC-CRR を v0.32 → v0.33(CR-0024 CLOSED + 本体 `9beeba2` + CI run `25603964726` 全 9 ジョブ Pass)、CI-DOC-DEVSTEPS を v0.41 → v0.42 に昇格、**1 コミットのみで初回 CI 全 Pass 成立**(Step 33-36 で 4 ステップ連続)) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.33(Step 13〜36 で CR-0001〜0024 を §4 本体表に登録 + Step 36 SHA 追記事後処理完了。**CR-0001〜0024 全 CLOSED**(CR-0024 は本体 `9beeba2` で全 9 ジョブ Pass、CI run `25603964726` + `25603964720`)。24 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:24 / B:3 / D:9 / **E:7** / **F:23**。集計表に Step 13〜36 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.35)§5/§6/§7 を参照。
