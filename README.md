...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.13(Step 17 で初版、Step 18〜28 で v0.2〜v0.12、Step 29 / CR-0017 で v0.13 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 309 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24(Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure → Hardware Simulator 層完成))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` + `test_electron_gun_sim.cpp` + `test_bending_magnet_sim.cpp` + `test_turntable_sim.cpp` + `test_ion_chamber_sim.cpp` で実装済**。後続 Step 30+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.21(Phase 4(Inc.1)Step 29 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-208 実装段階に + **CI-SRC-005 を UNIT-301 + UNIT-302 + UNIT-303 から UNIT-301 + UNIT-302 + UNIT-303 + UNIT-304 に拡大(Hardware Simulator 層 4 ユニット目 / 本層最終 / 故障注入機能あり)→ Hardware Simulator 層完成**、CI-DOC-UTPR を v0.12 → v0.13 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.19(Step 13〜29 で CR-0001〜0017 を §4 本体表に登録。17 件すべて MAJOR・安全関連、CR-0001〜0016 CLOSED + CR-0017 VERIFICATION(UNIT-304 + UTPR v0.13、**Hardware Simulator 層 4 ユニット目 / 本層最終 / 仮想イオンチェンバ応答模擬 + 故障注入機能 Saturation/ChannelFailure → Hardware Simulator 層完成**)。Therac-25 主要因類型: 累積 A:17 / B:3 / **D:9 新規**(UNIT-304 で故障注入 3 モード + 部分故障 + 混在故障モードによる IT/ST 試験ハーネス基盤を Sim 層に拡大)/ E:2 / **F:17 新規**(UNIT-304 で `static_assert` 二重表明(double + FaultMode)を 13 ユニット目に拡大、コード実装範囲内全 13 ユニット展開完了)。集計表に Step 13〜29 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)が成立、Step 24〜29 では Step 23/26〜28 教訓の継続反映により PRB 起票なし(6 連続クリーンコミット成立予定)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.21)§5/§6/§7 を参照。
