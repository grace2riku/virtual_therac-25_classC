...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.5(Step 17 で初版、Step 18〜20 で v0.2〜v0.4、Step 21 / CR-0009 で v0.5 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 106 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26(HZ-001 直接対応)+ UNIT-203 19(RCM-017 中核 / 許可フラグ消費型))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` で実装済**。後続 Step 22+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.13(Phase 4(Inc.1)Step 21 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-203 実装段階(**HZ-001 直接対応の中核 + RCM-017 中核**)に昇格、CI-DOC-UTPR を v0.4 → v0.5 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.11(Step 13〜21 で CR-0001〜0009 を §4 本体表に登録。9 件すべて MAJOR・安全関連、CI 全 Pass 確認済(CR-0009 は UNIT-203 + UTPR v0.5、**RCM-017 中核 / 許可フラグ消費型**, CLOSED)。Therac-25 主要因類型: 累積 A:9 / B:2 / **D:5 新規** / E:2 / F:9。集計表に Step 13〜21 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.13)§5/§6/§7 を参照。
