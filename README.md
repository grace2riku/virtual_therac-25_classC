...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.2(Step 17 / CR-0005 で初版作成、Step 18 / CR-0006 で v0.2 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証(SRMP §6.1 整合)、§5 受入基準 5 項目 + §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目を確定。**§7 UNIT-200 試験ケース 24 件 + UNIT-401 試験ケース 13 件 = 累積 37 件を定義し、`tests/unit/test_common_types.cpp` + `test_in_process_queue.cpp` で実装済**。後続 Step 19+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.10(Phase 4(Inc.1)Step 18 完了時点。`inc1-requirements-frozen`(BL-20260426-001)+ `inc1-design-frozen`(BL-20260426-002)確定済。SOUP 13 件正式登録(vcpkg ベースライン commit `c3867e714d...` で完全固定)、CI-SRC-001 を UNIT-200+UNIT-401 実装段階に昇格、CI-DOC-SDD を v0.1 → v0.1.1(軽微改訂)に、CI-DOC-UTPR を v0.1 → v0.2 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.8(Step 13/14/15/16/17/18 で CR-0001(SRS, CLOSED)/ CR-0002(C++20 骨格, CLOSED)/ CR-0003(SAD, CLOSED)/ CR-0004(SDD + `inc1-design-frozen`, CLOSED)/ CR-0005(UNIT-200 + UTPR v0.1, CLOSED)/ CR-0006(UNIT-401 + SDD v0.1.1 + UTPR v0.2, VERIFICATION)を §4 本体表に登録。6 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:6 / B:2 / D:2 / E:2 / F:6。集計表に Step 13〜18 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.10)§5/§6/§7 を参照。
