...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.4(Phase 3 完了時点。`planning-baseline`(BL-20260425-001)確定。ドキュメント 22 件、SOUP 候補 19 件、ツール 7 件運用中、構成定義 8 件運用中、TSan ランタイムを HZ-002 直接対応として明示分類、§10.1 に planning-baseline 詳細スナップショットを新設) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.2(空台帳。CCB 施行(2026-04-24)後も Phase 3 は立ち上げ活動の延長として運用、本格運用は Inc.1 着手時から開始。Therac-25 事故主要因類型 A〜F 別件数集計表を新設) |
