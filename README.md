...
| 5.4 | ソフトウェア詳細設計書 | [`5.4_.../software_detailed_design.md`](./5.4_software_detailed_design/software_detailed_design.md) | 🟡 v0.1(Inc.1 範囲: 全 22 ユニット — Inc.1 中核 14(UNIT-200/201〜206/208/301〜304/401〜402)+ Inc.2 空殻 1(UNIT-207)+ Inc.4 空殻 7(UNIT-101〜104/209/210)を確定。**§4 各 UNIT 詳細設計(IEC 62304 §5.4.2 クラス C 必須)**、**§5 IF 詳細設計(§5.4.3 クラス C 必須、メッセージ型 + Protocol Buffers スキーマ)**、**§8 詳細設計の検証(§5.4.4 クラス C 必須)** を全章カバー。`LifecycleState` 8 状態機械、`ErrorCode` 8 系統階層、`PulseCounter` `std::atomic<uint64_t>` で HZ-003 整数オーバフロー構造的排除、`folly::ProducerConsumerQueue` SPSC 採用。`inc1-design-frozen`(BL-20260426-002)で凍結) |
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.1(Step 17 / CR-0005 で初版作成。第 I 部 計画 + 第 II 部 報告 を初期化。§3 実装ルール(MISRA C++ 2023 + CERT C++ + clang-tidy + Sanitizer 3 種 + `std::atomic` 共有変数規則)、§4.2 並行処理 RCM の 3 層検証(TSan + 境界値 + モデル検査、SRMP §6.1 整合)、§5 受入基準 5 項目 + §6 クラス C 追加受入基準 9 項目 + §6.1 本プロジェクト固有追加基準 3 項目を確定。**§7 UNIT-200 試験ケース 24 件を定義し `tests/unit/test_common_types.cpp` で実装済**。後続 Step 18+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.9(Phase 4(Inc.1)Step 17 完了時点。`inc1-requirements-frozen`(BL-20260426-001)+ `inc1-design-frozen`(BL-20260426-002)確定済。SOUP 13 件正式登録(vcpkg ベースライン commit `c3867e714d...` で完全固定)、CI-SRC-001 を UNIT-200 CommonTypes 実装段階に昇格、CI-DOC-UTPR を v0.1 に昇格、CI-CFG-010(`.clang-tidy`)に Step 17 微調整(ClassCase 緩和)を反映) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.7(Step 13/14/15/16/17 で CR-0001(SRS, CLOSED)/ CR-0002(C++20 骨格, CLOSED)/ CR-0003(SAD, CLOSED)/ CR-0004(SDD + `inc1-design-frozen`, CLOSED)/ CR-0005(UNIT-200 + UTPR v0.1, VERIFICATION)を §4 本体表に登録。5 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:5 / B:2(構造的排除)/ D:2 / E:2 / F:5 いずれも構造的予防または構造的排除/前提。集計表に Step 13〜17 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.9)§5/§6/§7 を参照。
