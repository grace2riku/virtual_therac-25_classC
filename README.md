...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.5(Phase 4(Inc.1)着手時点。`inc1-requirements-frozen`(BL-20260426-001)確定。ドキュメント 22 件(SRS v0.1 含む)、SOUP 候補 19 件、ツール 7 件運用中、構成定義 8 件運用中、TSan ランタイムを HZ-002 直接対応として明示分類、§10.1 planning-baseline / §10.2 inc1-requirements-frozen 詳細スナップショットを新設) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.3(本格運用開始版。Step 13(2026-04-26)で CR-0001(Inc.1 SRS 作成、MAJOR、HZ-001/005 + 構造的 RCM-018/019)を §4 本体表に登録。Therac-25 事故主要因類型 A〜F 別件数集計に Step 13 行追加) |
