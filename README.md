...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.7(Phase 4(Inc.1)SAD 完成時点。`inc1-requirements-frozen`(BL-20260426-001)確定済。SOUP 13 件正式登録(vcpkg ベースライン commit `c3867e714d...` で完全固定、TSan ランタイムを HZ-002 直接対応として CI 常時実行)、ツール 8 件運用中、構成定義 6 件運用中、ドキュメント表で SAD v0.1 を反映、§10.1 planning-baseline / §10.2 inc1-requirements-frozen 詳細スナップショット) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.5(Step 13/14/15 で CR-0001(Inc.1 SRS、CLOSED)/ CR-0002(Inc.1 C++20 骨格、CLOSED)/ CR-0003(Inc.1 SAD、VERIFICATION)を §4 本体表に登録。3 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:3 / D:1 / E:1 / F:3 いずれも構造的予防または構造的前提。集計表に Step 13/14/15 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.7)§5/§6/§7 を参照。
