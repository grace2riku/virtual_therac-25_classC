...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.34(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28、Step 40 SHA 追記事後処理で v0.29、Step 41 / CR-0028 で v0.30、Step 42 で v0.31、Step 43 / CR-0029 で v0.32、Step 43 SHA 追記事後処理で v0.33、**Step 44 / CR-0030(UNIT-201 SafetyEventObserver 実装 + UNIT-203 BeamController dispatch、SDD §4.5 サンプル UT 粒度完成)で v0.33 → v0.34 昇格**(§3.2 UNIT-201 行に Step 44 範囲拡張の注記追加 + §7.2 UT-201-25〜28 4 件追加(累積 UNIT-201 28 件、UTPR 累積 414 件)、§11 改訂履歴 v0.34)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 414 件を定義**、後続 Step Inc.1 後半 / Step Inc.1 完了で Manager/Sim 結線 + IT-101 < 10 ms 実時間実測 + ITPR 作成を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.45(Phase 4(Inc.1)Step 44 完了時点(CR-0030 UNIT-201 SafetyEventObserver 実装 + UNIT-203 BeamController dispatch、SDD §4.5 サンプル「目標到達 → BeamOff < 1 ms 連鎖」UT 粒度完成、HZ-005 直接対応 + RCM-017 経路完成、UT-201-25〜28 4 件追加 累積 414 件、本番コード変更ありだが SAD/SDD 不変 `inc1-design-frozen` 維持)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 UNIT-201 拡張、CI-DOC-UTPR を v0.33 → v0.34、CI-DOC-CRR を v0.42 → v0.43(CR-0030 第 30 例登録、状態 OPEN)、CI-DOC-DEVSTEPS を v0.51 → v0.52) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.43(Step 13〜44 で CR-0001〜0030 を §4 本体表に登録。**CR-0001〜0029 全 CLOSED + CR-0030 OPEN**(CR-0030 は Step 44 UNIT-201 SafetyEventObserver 実装 + UNIT-203 BeamController dispatch、本格運用第 30 例、本 push 後の CI Pass 後の SHA 追記事後処理で CLOSED 化予定)。30 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:28 / B:3 / D:10 / **E:9** / **F:29**。集計表に Step 13〜44 完了時行を追加。**PRB-0007 + PRB-0008 起票 2 件**(SPRP §3.1 本格運用第 5 例 + 第 6 例、新類型 2 件: 「他ユニット UT テンプレ流用時の Severity マッピング自己セルフチェック漏れ」+「並行 UT 追加時の target 値 SRS-008 範囲内セルフチェック漏れ」)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.45)§5/§6/§7 を参照。
