...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.31(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28、Step 40 SHA 追記事後処理で v0.29、Step 41 / CR-0028 で v0.30(SafetyEventObserver(共通基盤)+ UNIT-204 拡張 + UT-204-33〜37 5 件追加、累積 410 件)、**Step 42(独立 Step、Step 41 SHA 追記事後処理 + ストリーク復活第 2 例途絶記録 + PRB-0008 起票候補記録)で v0.30 → v0.31 昇格**(§8.1duovicies コミット欄を本体 `0fc22b7` + 修正 `17295c6` に確定 + CI 結果「全 9 ジョブ success ✅(run id `25773208383` + `25773208389`)」確定 + UT-204-37 race detection 0 確定 + ストリーク復活第 2 例途絶記録 + PRB-0008 起票候補記録、§11 改訂履歴 v0.31)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 410 件を定義**、後続 Step 42+ で UNIT-201 SafetyCoreOrchestrator 結線 + UNIT-203 BeamController dispatch を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.42(Phase 4(Inc.1)Step 42 完了時点(独立 Step、Step 41 SHA 追記事後処理 + Step 33-36 連続 1 コミット初回 CI Pass ストリーク復活第 2 例途絶記録 + PRB-0008 起票候補記録、Step 38 同パターン)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-DOC-UTPR を v0.30 → v0.31、CI-DOC-CRR を v0.39 → v0.40(CR-0028 CLOSED + 本体 `0fc22b7` + 修正 `17295c6` + CI run id + Sanitizer 全 Pass 確定)、CI-DOC-DEVSTEPS を v0.48 → v0.49) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.40(Step 13〜41 で CR-0001〜0028 を §4 本体表に登録。**CR-0001〜0028 全 CLOSED**(CR-0028 は Step 41 SafetyEventObserver 基盤導入、本格運用第 28 例、本 Step 42 SHA 追記事後処理で OPEN → CLOSED 遷移完了、本体 `0fc22b7` + 修正 `17295c6` + CI run id `25773208383` / `25773208389` 確定 + Sanitizer 全 Pass 確定 + クローズ日 2026-05-13)。28 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:27 / B:3 / D:9 / **E:9** / **F:27**。集計表に Step 13〜41 完了時行を追加(Step 41 完了時行は実績値に更新、ストリーク復活第 2 例途絶 + PRB-0008 起票候補記録)。**PRB-0007 起票 1 件**(SPRP §3.1 本格運用第 5 例)+ **PRB-0008 起票候補**(新類型「並行 UT 追加時の target 値 SRS-008 範囲内セルフチェック漏れ」、Step 43+ で起票予定)) |
