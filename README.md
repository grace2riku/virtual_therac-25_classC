...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.8(Step 17 で初版、Step 18〜23 で v0.2〜v0.7、Step 24 / CR-0012 で v0.8 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 194 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31(HZ-005 部分対応 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` で実装済**。後続 Step 25+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.16(Phase 4(Inc.1)Step 24 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-206 実装段階(**HZ-001 直接対応の中核 + RCM-017 中核 + HZ-005 直接 + HZ-003 構造的排除の中核 + RCM-005 中核 / 3 系統センサ集約 + RCM-008 中核 / 強い型による単位安全 + エネルギー対応表**)に昇格、CI-DOC-UTPR を v0.7 → v0.8 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.14(Step 13〜24 で CR-0001〜0012 を §4 本体表に登録。12 件すべて MAJOR・安全関連、CR-0001〜0009 CLOSED + CR-0010〜0012 VERIFICATION(UNIT-204/UNIT-205/UNIT-206 + UTPR v0.6/v0.7/v0.8、**HZ-005 直接 + HZ-003 構造的排除 + HZ-001 直接 + RCM-005/008 中核**)。Therac-25 主要因類型: 累積 A:12 / B:3 / D:6 / E:2 / **F:12 新規**(UNIT-206 で `static_assert` 二重表明を 8 ユニット目に拡大、コード実装範囲内全 8 ユニットに展開完了)。集計表に Step 13〜24 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)が成立) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.16)§5/§6/§7 を参照。
