...
| 5.3 | ソフトウェアアーキテクチャ設計書 | [`5.3_.../software_architecture_design.md`](./5.3_software_architecture_design/software_architecture_design.md) | 🟡 v0.1(Inc.1 範囲: ARCH-001〜004 14 項目、IF-U-001〜010、IF-E-001〜004、SEP-001/002/003 を確定。**§9 SEP-001(UI 層と安全コアの OS プロセス分離)+ SEP-003(共有変数禁止 + lock-free queue)で Therac-25 race condition の根本構造を構造的に不可能化**。§7/§8 で SOUP 13 件の機能・性能要求 + HW/SW 要求を割付(IEC 62304 §5.3.3/§5.3.4 クラス B/C 必須)。`inc1-design-frozen` は Step 16 SDD 完了時に確定予定) |
| 5.4 | ソフトウェア詳細設計書 | [`5.4_.../software_detailed_design.md`](./5.4_software_detailed_design/software_detailed_design.md) | 🟡 v0.1.1(Inc.1 範囲: 全 22 ユニット。**§4 各 UNIT 詳細設計(IEC 62304 §5.4.2 クラス C 必須)**、**§5 IF 詳細設計(§5.4.3 クラス C 必須)**、**§8 詳細設計の検証(§5.4.4 クラス C 必須)** を全章カバー。**v0.1.1(Step 18 / CR-0006)で UNIT-401 の実装手段を `folly::ProducerConsumerQueue` 採用 → 独自 SPSC 実装(Lamport 1983)に確定**(SDD §10 未確定事項のクローズ、`inc1-design-frozen`(BL-20260426-002)維持)。`LifecycleState` 8 状態機械、`ErrorCode` 8 系統階層、`PulseCounter` `std::atomic<uint64_t>` で HZ-003 構造的排除) |
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.35(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28、Step 40 SHA 追記事後処理で v0.29、Step 41 / CR-0028 で v0.30、Step 42 で v0.31、Step 43 / CR-0029 で v0.32、Step 43 SHA 追記事後処理で v0.33、Step 44 / CR-0030 で v0.34、**Step 44 SHA 追記事後処理(Step 40 パターン、独立 Step 番号なし)で v0.34 → v0.35 昇格**(§8.4 Step 44 / CR-0030 範囲行を実績値に更新: 本体 `4537a40` 確定 + CI run id `25787257539` C++ Build 全 9 ジョブ Pass ✅ + `25787256096` Documentation Checks success ✅ 確定 + 1 コミット初回 CI Pass 復活第 3 例成立追記、§11 改訂履歴 v0.35)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 414 件を定義**、後続 Step Inc.1 後半 / Step Inc.1 完了で Manager/Sim 結線 + IT-101 < 10 ms 実時間実測 + ITPR 作成を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.46(Phase 4(Inc.1)Step 44 SHA 追記事後処理時点(Step 40 パターン、独立 Step 番号なし、CR-0030 状態 OPEN → CLOSED 遷移完了、本体 `4537a40` + CI run id `25787257539`(C++ Build (Inc.1) 全 9 ジョブ Pass ✅、9 分 30 秒、2026-05-13)+ `25787256096`(Documentation Checks success ✅、21 秒)+ Sanitizer 全 Pass + クローズ日 2026-05-13 確定、**1 コミット初回 CI Pass 復活第 3 例成立**)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-DOC-UTPR を v0.34 → v0.35、CI-DOC-CRR を v0.43 → v0.44(CR-0030 CLOSED + 本体 SHA + CI run id 確定)、CI-DOC-DEVSTEPS を v0.52 → v0.53) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.44(Step 13〜44 で CR-0001〜0030 を §4 本体表に登録。**CR-0001〜0030 全 CLOSED**(本 Step 44 SHA 追記事後処理で CR-0030 状態 OPEN → CLOSED 遷移完了、本体 `4537a40` + CI run id `25787257539`(C++ Build (Inc.1) 全 9 ジョブ Pass ✅、9 分 30 秒、2026-05-13)+ `25787256096`(Documentation Checks success ✅、21 秒)+ Sanitizer 全 Pass + クローズ日 2026-05-13)。30 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:28 / B:3 / D:10 / **E:9** / **F:29**。集計表 Step 44 完了時行を実績値に更新(**1 コミット初回 CI Pass 復活第 3 例成立**、Step 40 第 1 例 → Step 43 第 2 例 → 本 Step 44 第 3 例、サイクル 2 周目完成後の継続)。**PRB-0007 + PRB-0008 起票 2 件**(SPRP §3.1 本格運用第 5 例 + 第 6 例、新類型 2 件: 「他ユニット UT テンプレ流用時の Severity マッピング自己セルフチェック漏れ」+「並行 UT 追加時の target 値 SRS-008 範囲内セルフチェック漏れ」)) |
| 9 問題解決 | ソフトウェア問題解決手順書(SPRP) | [`9_.../software_problem_resolution_procedure.md`](./9_software_problem_resolution_process/software_problem_resolution_procedure.md) | 🟡 v0.3(GitHub Issue ベースの PRB-NNNN 運用、根本原因 13 分類、Therac-25 事故 5 主要因チェックリストを Critical 案件で必須化、傾向分析に Therac-25 主要因類型別集計を組込、Step 39 / CR-0026 で v0.1 → v0.2(PRB-0007 起票記録 + 教訓水平展開運用ルール本格運用第 2 例)、**Step 43 / CR-0029 で v0.2 → v0.3 昇格**(§12 改訂履歴に PRB-0008 起票記録 + 教訓水平展開運用ルール本格運用第 3 例 + 新類型「並行 UT 追加時の target 値 SRS 範囲内セルフチェック漏れ」を §8.2 分析項目への運用上の追加類型として組込)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.46)§5/§6/§7 を参照。
