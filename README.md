...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.10(Step 17 で初版、Step 18〜25 で v0.2〜v0.9、Step 26 / CR-0014 で v0.10 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 242 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19(Hardware Simulator 層の最初の具体実装 / 仮想電子銃応答模擬))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` + `test_electron_gun_sim.cpp` で実装済**。後続 Step 27+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.18(Phase 4(Inc.1)Step 26 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-208 実装段階に + **CI-SRC-005 を Step 14 プレースホルダから UNIT-301 ElectronGunSim 実装段階に新規昇格(Hardware Simulator 層の最初の具体実装)**、CI-DOC-UTPR を v0.9 → v0.10 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.16(Step 13〜26 で CR-0001〜0014 を §4 本体表に登録。14 件すべて MAJOR・安全関連、CR-0001〜0013 CLOSED + CR-0014 VERIFICATION(UNIT-301 + UTPR v0.10、**Hardware Simulator 層の最初の具体実装 / 仮想電子銃応答模擬**)。Therac-25 主要因類型: 累積 A:14 / B:3 / D:7 / E:2 / **F:14 新規**(UNIT-301 で `static_assert` 三重表明を 10 ユニット目に拡大)。集計表に Step 13〜26 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)が成立、Step 24/25/26 では Step 23 教訓の継続反映により PRB 起票なし) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.18)§5/§6/§7 を参照。
