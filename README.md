...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.22(Step 17 で初版、Step 18〜33 で v0.2〜v0.18、Step 34 / CR-0022 で v0.19、Step 34 SHA 追記事後処理で v0.20、Step 35 / CR-0023 で v0.21、Step 35 SHA 追記事後処理で v0.22 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 369 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12(Inc.4 で本格化予定のビームオン/オフ操作 UI ユニット、空殻実装としては第 5 例、既存ライブラリ `th25_ui` の 2 cpp 目組込))**、`test_beam_command_console.cpp` 新規追加。後続 Step 36+ で UNIT-103/104 / UNIT-402 等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.33(Phase 4(Inc.1)Step 35 完了 + SHA 追記事後処理時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-004 を Step 35 で UNIT-101 + UNIT-102 BeamCommandConsole 空殻に拡大**(`th25_ui` ライブラリの 2 cpp 目組込、新ライブラリ追加なし、空殻実装としては第 5 例)、CI-SRC-001 / CI-SRC-005 は変更なし、CI-DOC-UTPR v0.22、CI-DOC-CRR を v0.30 → v0.31(CR-0023 CLOSED + 本体 `4e9f4d1` + CI run `25592176524` 全 9 ジョブ Pass)、CI-DOC-DEVSTEPS を v0.39 → v0.40 に昇格、**1 コミットのみで初回 CI 全 Pass 成立**(Step 33 / Step 34 と同様)) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.31(Step 13〜35 で CR-0001〜0023 を §4 本体表に登録 + Step 35 SHA 追記事後処理完了。**CR-0001〜0023 全 CLOSED**(CR-0023 は本体 `4e9f4d1` で全 9 ジョブ Pass、CI run `25592176524` + `25592176527`)。23 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:23 / B:3 / D:9 / **E:6** / **F:22**。集計表に Step 13〜35 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.33)§5/§6/§7 を参照。
