...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.32(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25、Step 37 SHA 追記事後処理で v0.26、Step 39 / CR-0026 で v0.27、Step 40 / CR-0027 で v0.28、Step 40 SHA 追記事後処理で v0.29、Step 41 / CR-0028 で v0.30、Step 42 で v0.31、**Step 43 / CR-0029(教訓水平展開 / 改善 CR、PRB-0008 連動)で v0.31 → v0.32 昇格**(§8.4 表に PRB-0008(#37)行 + Step 43 / CR-0029 範囲行追加(全 7 UT ファイル一斉点検結果: Step 41 修正済 UT-204-37 以外に不整合なし、本番コード / UT コード変更なし、運用ルール追加 + ドキュメント整合化のみ)、§11 改訂履歴 v0.32)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 410 件を定義**、後続 Step 44+ で UNIT-201 SafetyCoreOrchestrator 結線 + UNIT-203 BeamController dispatch を実施予定) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.43(Phase 4(Inc.1)Step 43 完了時点(CR-0029 教訓水平展開 / 改善 CR、PRB-0008 連動、CLAUDE.md「UT 作成時の二重セルフチェック (Severity マッピング + SRS 範囲内)」統合節集約、SPRP §3.1 教訓水平展開運用ルール本格運用第 3 例、本番コード / UT コード変更なし、運用ルール追加 + ドキュメント整合化のみ)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-DOC-UTPR を v0.31 → v0.32、CI-DOC-CRR を v0.40 → v0.41(CR-0029 第 29 例登録、状態 OPEN)、CI-DOC-SPRP を v0.2 → v0.3、CI-DOC-DEVSTEPS を v0.49 → v0.50、CI-DOC-CLAUDE 行に統合節集約反映) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.41(Step 13〜43 で CR-0001〜0029 を §4 本体表に登録。**CR-0001〜0028 全 CLOSED + CR-0029 OPEN**(CR-0029 は Step 43 教訓水平展開 / 改善 CR、PRB-0008 連動、本格運用第 29 例、本番コード / UT コード変更なし、運用ルール追加 + ドキュメント整合化のみ、CI は Documentation Checks のみ実行見込み、クローズは本 push 後の CI Pass 後の SHA 追記事後処理で確定予定)。29 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:27 / B:3 / D:9 / **E:9** / **F:28**。集計表に Step 13〜43 完了時行を追加。**PRB-0007 + PRB-0008 起票 2 件**(SPRP §3.1 本格運用第 5 例 + 第 6 例、新類型 2 件: 「他ユニット UT テンプレ流用時の Severity マッピング自己セルフチェック漏れ」+「並行 UT 追加時の target 値 SRS-008 範囲内セルフチェック漏れ」)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.43)§5/§6/§7 を参照。
