...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.39(Phase 4(Inc.1)Step 40 完了時点(CR-0027 UNIT-402 InterProcessChannel 空殻実装、Inc.4 で本格化、空殻実装第 8 例)。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-001 を UNIT-200〜UNIT-210 から UNIT-200〜UNIT-210 + UNIT-402 InterProcessChannel 空殻に拡大**(`th25_ctrl` ライブラリの 12 cpp 目組込、空殻実装第 8 例、HZ-006 中核 + RCM-018 中核 IPC ラッパ、CLAUDE.md / CR-0026 制定の Severity マッピング自己セルフチェック運用ルール初適用例)、CI-DOC-UTPR を v0.27 → v0.28、CI-DOC-CRR を v0.36 → v0.37(CR-0027 登録、本格運用第 27 例)、CI-DOC-DEVSTEPS を v0.45 → v0.46) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.37(Step 13〜40 で CR-0001〜0027 を §4 本体表に登録。**CR-0001〜0026 全 CLOSED + CR-0027 OPEN**(CR-0027 は Step 40 UNIT-402 InterProcessChannel 空殻実装、本格運用第 27 例、push 後 SHA 追記事後処理で CLOSED 化予定)。27 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:26 / B:3 / D:9 / **E:9** / **F:26**(CR-0027 で A/E/F 系 +1)。集計表に Step 13〜40 完了時行を追加。**PRB-0007 起票 1 件**(SPRP §3.1 本格運用第 5 例)) |
