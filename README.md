...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.14(Step 17 で初版、Step 18〜29 で v0.2〜v0.13、Step 30 / CR-0018 で v0.14 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 321 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12(Inc.2 で本格化予定の独立並行タスクのインターロック監視ユニット、空殻実装としては第 1 例))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` + `test_electron_gun_sim.cpp` + `test_bending_magnet_sim.cpp` + `test_turntable_sim.cpp` + `test_ion_chamber_sim.cpp` + `test_safety_monitor.cpp` で実装済**。後続 Step 31+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.23(Phase 4(Inc.1)Step 30 完了 + SHA 追記事後処理 + PRB-0005 反映時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-001 を UNIT-200/401/201/202/203/204/205/206/208 から + UNIT-207 SafetyMonitor 空殻(Inc.2 で本格化予定 / 独立並行タスクのインターロック監視 / 空殻実装としては第 1 例)に拡大、`th25_ctrl` ライブラリの 9 cpp 目として組込**、CI-SRC-005 は UNIT-301〜304 で Hardware Simulator 層完成済、CI-DOC-UTPR v0.14、CI-DOC-CRR を v0.20 → v0.21(CR-0018 CLOSED + PRB-0005 反映)、CI-DOC-DEVSTEPS を v0.29 → v0.30 に昇格、SPRP §3.1 PRB 起票プロセス本格運用第 3 例) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.21(Step 13〜30 で CR-0001〜0018 を §4 本体表に登録 + Step 30 SHA 追記事後処理 + PRB-0005 反映完了。18 件すべて MAJOR・安全関連、CR-0001〜0018 全 CLOSED(CR-0018 は本体 `84276cc` + PRB-0005 修正 `12008b7` で全 9 ジョブ Pass、CI run `25446778687`)。Therac-25 主要因類型: 累積 A:18 / B:3 / D:9(本 v0.14 では IF 骨格 + 並行設計の構造のみ、Inc.2 で本格化)/ E:2 / **F:18 新規**(UNIT-207 で `static_assert` 二重表明を **14 ユニット目に拡大、Inc.1 範囲全 14 ユニット展開完了**)。集計表に Step 13〜30 完了時行を追加。**Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)、Step 30 で第 3 例(PRB-0005、UT-208-29 gcc-13/debug 環境固有 flaky を do-while パターン化で構造的予防)が成立**。Step 24〜29 では Step 23/26〜28 教訓の継続反映により PRB 起票なし(Step 30 で UT-208-29 既存テストの設計上の脆弱性顕在化により 7 連続クリーンコミットは不成立、ただし空殻実装本体は初版即ビルド成功)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.23)§5/§6/§7 を参照。
