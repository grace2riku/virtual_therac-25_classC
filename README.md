...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.18(Step 17 で初版、Step 18〜31 で v0.2〜v0.15、Step 32 / CR-0020 で v0.16 昇格、v0.17 で Step 32 SHA 追記事後処理 + PRB-0006 反映、**v0.18 で Step 33 / CR-0021 教訓水平展開 / 改善 CR を反映**(全 UT 並行試験 11 箇所を `while` → `do-while` パターンに一斉修正、PRB-0005 / PRB-0006 同根本原因の予防的水平展開、SPRP §3.1 PRB 起票プロセスの教訓水平展開運用ルールの本格運用初例)。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 345 件を定義は不変**(本 CR は試験設計改善のみで新規 UT 追加なし)。後続 Step 34+ で UNIT-101〜104 / UNIT-402 等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.29(Phase 4(Inc.1)Step 33 完了 + SHA 追記事後処理時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 / CI-SRC-005 ともに本 Step では本体ソース変更なし(Step 33 / CR-0021 は試験ハーネス改善のみ)、CI-DOC-UTPR v0.18、CI-DOC-CRR を v0.26 → v0.27(CR-0021 CLOSED + 本体 `e19fe9d` + CI run `25562291211` 全 9 ジョブ Pass)、CI-DOC-DEVSTEPS を v0.35 → v0.36 に昇格、**Step 33 / CR-0021 で教訓水平展開 / 改善 CR が完成**(全 UT 並行試験 11 箇所を do-while パターン化、SPRP §3.1 PRB 起票プロセスの教訓水平展開運用ルールの本格運用初例完成、1 コミットのみで初回 CI 全 Pass 成立、Step 32 PRB-0006 で一時中断したクリーンコミット記録を再達成)) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.27(Step 13〜33 で CR-0001〜0021 を §4 本体表に登録 + Step 33 SHA 追記事後処理完了。**CR-0001〜0021 全 CLOSED**(CR-0021 は本体 `e19fe9d` で全 9 ジョブ Pass、CI run `25562291211` + `25562291215`)。21 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:21 / B:3 / D:9 / E:4 / F:20。集計表に Step 13〜33 完了時行を追加。**Step 22 で SPRP §3.1 PRB 起票プロセス本格運用第 1 例、Step 23 で第 2 例、Step 30 で第 3 例、Step 32 で第 4 例(Step 30 PRB-0005 と完全同一の根本原因)が成立、Step 33 で教訓水平展開運用ルールの本格運用初例として CR-0021 を完成**(全 UT 並行試験 11 箇所を do-while パターン化 + 1 コミットのみで初回 CI 全 Pass、PRB-0007 以降の同根本原因再発を構造的予防)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.29)§5/§6/§7 を参照。
