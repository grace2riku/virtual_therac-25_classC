...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.12(Step 17 で初版、Step 18〜27 で v0.2〜v0.11、Step 28 / CR-0016 で v0.12 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 285 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24(Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse))、`test_common_types.cpp` + `test_in_process_queue.cpp` + `test_safety_core_orchestrator.cpp` + `test_treatment_mode_manager.cpp` + `test_beam_controller.cpp` + `test_dose_manager.cpp` + `test_turntable_manager.cpp` + `test_bending_magnet_manager.cpp` + `test_startup_self_check.cpp` + `test_electron_gun_sim.cpp` + `test_bending_magnet_sim.cpp` + `test_turntable_sim.cpp` で実装済**。後続 Step 29+ で他ユニットを順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.20(Phase 4(Inc.1)Step 28 完了時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、CI-SRC-001 を UNIT-200〜UNIT-208 実装段階に + **CI-SRC-005 を UNIT-301 + UNIT-302 から UNIT-301 + UNIT-302 + UNIT-303 に拡大(Hardware Simulator 層 3 ユニット目 / 故障注入機能あり)**、CI-DOC-UTPR を v0.11 → v0.12 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.18(Step 13〜28 で CR-0001〜0016 を §4 本体表に登録。16 件すべて MAJOR・安全関連、CR-0001〜0015 CLOSED + CR-0016 VERIFICATION(UNIT-303 + UTPR v0.12、**Hardware Simulator 層 3 ユニット目 / 仮想ターンテーブル応答模擬 + 故障注入機能 StuckAt/Delay/NoResponse**)。Therac-25 主要因類型: 累積 A:16 / B:3 / **D:8 新規**(UNIT-303 で故障注入 4 モード + 部分故障 + 混在故障モードによる IT/ST 試験ハーネス基盤を Sim 層で初実装)/ E:2 / **F:16 新規**(UNIT-303 で `static_assert` 四重表明(double + bool + uint32_t + FaultMode)を 12 ユニット目に拡大)。集計表に Step 13〜28 完了時行を追加。Step 22 で SPRP §3.1 PRB 起票プロセスの本格運用第 1 例(PRB-0001/0002)、Step 23 で第 2 例(PRB-0003/0004)が成立、Step 24〜28 では Step 23/26/27 教訓の継続反映により PRB 起票なし(5 連続クリーンコミット成立予定)) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.20)§5/§6/§7 を参照。
