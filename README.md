...
| 5.5 | ユニット試験計画書/報告書 | [`5.5_.../software_unit_test_plan_report.md`](./5.5_software_unit_implementation/software_unit_test_plan_report.md) | 🟡 v0.25(Step 17 で初版、Step 18〜35 事後処理で v0.2〜v0.22、Step 36 / CR-0024 で v0.23、Step 36 SHA 追記事後処理で v0.24、Step 37 / CR-0025 で v0.25 昇格。§3 実装ルール、§4.2 並行処理 RCM の 3 層検証、§5/§6/§6.1 受入基準を確定。**§7 累積 393 件を定義(UNIT-200 24 + UNIT-401 13 + UNIT-201 24 + UNIT-202 26 + UNIT-203 19 + UNIT-204 32 + UNIT-205 25 + UNIT-206 31 + UNIT-208 29 + UNIT-301 19 + UNIT-302 19 + UNIT-303 24 + UNIT-304 24 + UNIT-207 12 + UNIT-209 12 + UNIT-210 12 + UNIT-101 12 + UNIT-102 12 + UNIT-103 12 + UNIT-104 12(Inc.4 で本格化予定の操作者認証 UI ユニット、空殻実装としては第 7 例、既存ライブラリ `th25_ui` の 4 cpp 目組込、HZ-010 構造的位置づけ + RCM-015 中核 UI 側ゲートウェイ、UNIT-210 と一対、Inc.1 範囲の全 UI 系 4 ユニット完成))**、`test_ui_authentication_gateway.cpp` 新規追加。後続 Step 38+ で UNIT-402 / Inc.1 後半 dispatch 機構等を順次追加) |
...
| 8 構成管理 | 構成アイテム一覧(CIL) | [`8_.../configuration_item_list.md`](./8_software_configuration_management_process/configuration_item_list.md) | 🟡 v0.36(Phase 4(Inc.1)Step 37 / CR-0025 時点。`inc1-requirements-frozen` + `inc1-design-frozen` 確定済。SOUP 13 件正式登録、**CI-SRC-004 を Step 37 で UNIT-101 + UNIT-102 + UNIT-103 + UNIT-104 UIAuthenticationGateway 空殻に拡大**(`th25_ui` ライブラリの 4 cpp 目組込、新ライブラリ追加なし、空殻実装としては第 7 例、HZ-010 構造的位置づけ + RCM-015 中核 UI 側ゲートウェイ、UNIT-210 と一対、Inc.1 範囲 UI 系 4 ユニット完成)、CI-SRC-001 / CI-SRC-005 は変更なし、CI-DOC-UTPR v0.25、CI-DOC-CRR を v0.33 → v0.34(CR-0025 OPEN、本体 SHA は事後処理で確定予定)、CI-DOC-DEVSTEPS を v0.42 → v0.43 に昇格) |
...
| 8 構成管理 | 変更要求台帳 | [`8_.../change_request_register.md`](./8_software_configuration_management_process/change_request_register.md) | 🟡 v0.34(Step 13〜37 で CR-0001〜0025 を §4 本体表に登録。**CR-0001〜0024 全 CLOSED + CR-0025 OPEN**(CR-0025 は本体 SHA TBD、push 後 SHA 追記事後処理で CLOSED 化予定)。25 件すべて MAJOR・安全関連。Therac-25 主要因類型: 累積 A:25 / B:3 / D:9 / **E:8** / **F:24**。集計表に Step 13〜36 完了時行を追加) |
...
詳細は SDP §6.2 および 構成アイテム一覧(CIL-TH25-001 v0.36)§5/§6/§7 を参照。
